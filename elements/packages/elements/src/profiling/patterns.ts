...
export function detectPatterns(analysis: ProfileAnalysis): Pattern[] {
  ...
  if (domManipulation.length > 0) {
    ...
    patterns.push({
      ...
      suggestion:
        "Consider batching DOM updates, using DocumentFragment, or reusing elements instead of creating/destroying",
    });
  }

  // Pattern 2: Layout thrashing (reading layout properties)
  const layoutReads = analysis.hotspots.filter(
    (h) =>
      h.functionName.match(
        /getBoundingClientRect|offsetWidth|offsetHeight|offsetTop|offsetLeft|scrollTop|scrollLeft|clientWidth|clientHeight/i,
      ) &&
      h.callCount &&
      h.callCount > 50,
  );
  ...
  const tightLoops = analysis.hotspots.filter(
    (h) => h.hitCount > 200 && h.selfTimePct > 5 && h.selfTimePct < 30,
  );
  if (tightLoops.length > 0) {
    patterns.push({
      ...
      hotspots: tightLoops,
      suggestion:
        "Review loop logic - consider reducing iterations, early exit, or moving work outside loop",
    });
  }

  // Pattern 4: Frequent function calls with low individual cost
  const frequentCalls = analysis.hotspots.filter(
    (h) =>
      h.callCount &&
      h.callCount > 100 &&
  ...
  if (jsonOperations.length > 0) {
    patterns.push({
      ...
      suggestion:
        "Consider using structured cloning, immutable data structures, or reducing serialization",
    });
  }
