...
export const executeInElectronWithRpc = async (script: string) => {
  const { electronProcess, processExit, socketPath } = await spawnElectronBootloader(script);
  ...
  try {
    await raceTimeout(30_000, `RPC server in electron process PID ${electronProcess.pid} not ready within 30 seconds`, rpcReady.promise);
  } catch (error) {
    logger.error({ pid: electronProcess.pid, stderr: stderrBuffer }, "Electron stderr during timeout");
    rpcReadySpan.setStatus({ code: SpanStatusCode.ERROR });
    rpcReadySpan.recordException(error instanceof Error ? error : new Error(String(error)));
    rpcReadySpan.end();
    throw error;
  }
...
const spawnElectronBootloader = async (script: string) => {
  return await tracer.startActiveSpan("spawnElectronBootloader", async (span) => {
    try {
      await ensureXvfb();
      ...
      const electronProcess = spawn("node_modules/.bin/electron", [
        "--no-sandbox",
        ...
        env: {
          ...
          OTEL_EXPORTER_OTLP_ENDPOINT: process.env.OTEL_EXPORTER_OTLP_ENDPOINT || "http://tracing:4318",
      ...
      electronProcess.stderr.on("data", (data) => {
        ...
        if (data.toString().includes("Exiting GPU process due to errors during initialization")) {
          return
        }
      ...
      const processExit = new Promise((resolve, reject) => {
        electronProcess.on("close", (code) => {
          ...
          if (code !== 0) {
            span.setStatus({ code: SpanStatusCode.ERROR });
            const error = new Error(`Electron process exited with code ${code}`);
            span.recordException(error);
            reject(error);
          } else {
            span.setStatus({ code: SpanStatusCode.OK });
