...
function TrackBackdrop({ width, opacity }: { width: number; opacity: number }) {
  return (
    <mesh position={[0, 0.02, 0]} receiveShadow>
      <boxGeometry args={[width, 0.4, TRACK_D + 0.02]} />
      ...
      {UNIFIED_CLIPS.map((clip, i) => {
        ...
        return (
          <TrackClip
            key={i}
            width={clipW}
            height={TRACK_H[clip.track]!}
            depth={TRACK_D}
            x={clipX}
            y={TRACK_Y[clip.track]!}
  ...
  return (
    <group position={position}>
      <mesh castShadow={opacity > 0.1} rotation={[0, rotation, 0]}>
        ...
        <lineSegments>
          <edgesGeometry
            args={[new THREE.BoxGeometry(NODE_SIZE, NODE_SIZE, NODE_SIZE)]}
          />
          <lineBasicMaterial
            color={COL_BLUE_LT}
            transparent
...
export function ParallelFragmentsR3FScene({
  ...
  const segJoinedX = (i: number) =>
    -TOTAL_W / 2 + SEG_W / 2 + i * (SEG_W + SEG_GAP);
  ...
  const seqScale = inP5 ? lerp(0.5, 0.35, p5) : inP4 ? lerp(1, 0.5, flyIn) : 1;
  const seqOpacity = inP5
    ? lerp(0.3, 0.12, p5)
    : inP4
      ? lerp(0.55, 0.3, processing)
      : seqAppear * 0.55;

  // ── Segment positions ──
  const gapOpen = inP2
    ? easeOut(prog(timeMs, P2_START + 200, P2_START + 700))
    : 0;
  ...
  const titleOpacity =
    easeOut(prog(timeMs, 300, 800)) * (1 - easeOut(prog(timeMs, 1800, 2200)));
  const questionOpacity =
    easeOut(prog(timeMs, 2200, 2600)) * (1 - easeOut(prog(timeMs, 2900, 3200)));
  const sideLabelsOpacity = easeOut(
    prog(timeMs, P2_START + 400, P2_START + 800),
  );
  const subLabelsOpacity = easeOut(
    prog(timeMs, P2_START + 700, P2_START + 1100),
  );
  const segDurationOpacity = inP2
    ? easeOut(prog(timeMs, P2_START + 600, P2_START + 1000))
    : 0;
  const processingOpacity =
    easeOut(prog(timeMs, P4_START + 1000, P4_START + 1400)) *
    (1 - easeOut(prog(timeMs, P4_PAR_DONE - 500, P4_PAR_DONE)));
  const punchlineOpacity = easeOut(
    prog(timeMs, P5_START + 200, P5_START + 600),
  );
  ...
  return (
    <>
      ...
      <SceneLabel
        position={[SEQ_X, 0.7, 0.5]}
        opacity={sideLabelsOpacity * (inP5 ? lerp(1, 0.2, p5) : 1)}
        color="#888888"
        fontSize={0.2}
        bold
      >
      ...
      <SceneLabel
        position={[SEQ_X, 0.55, 0.5]}
        opacity={subLabelsOpacity * (inP5 ? lerp(1, 0.15, p5) : 1)}
        color="#777777"
        fontSize={0.11}
      >
      ...
      <SceneLabel
        position={[PAR_X, 0.55, 0.5]}
        opacity={subLabelsOpacity}
        color="#aaaaaa"
        fontSize={0.11}
      >
          ...
          <DurationLabel
            ...
            x={
              inP4
                ? parLaneX
                : lerp(
                    lerp(segTightX(s), segJoinedX(s), gapOpen),
                    parLaneX,
                    slideRight,
                  )
            }
      ...
      {/* Punchline */}
      <SceneLabel
        position={[PAR_X + 0.3, -0.2, NODE_Z + 2.0 + (inP5 ? p5 * 0.6 : 0)]}
      ...
      <SceneLabel
        position={[PAR_X + 0.3, -0.5, NODE_Z + 2.0 + (inP5 ? p5 * 0.6 : 0)]}
      ...
      {[0, 1, 2].map((c) => (
        <LaserCutLine
          key={c}
          x={segTightX(c) + SEG_W / 2}
          y={0.3}
          opacity={cutBrightness(c)}
        />
      ))}
      {inP2 && (
        <pointLight
      ...
      {Array.from({ length: NUM_SEGS }, (_, s) => {
        ...
        const currentX = inP4
          ? parLaneX
          : lerp(lerp(tightX, gappedX, gapOpen), parLaneX, slideRight);
        const segY = inP4 ? lerp(0.3, -0.15, flyIn) : 0.3;
        const segZ = inP4
          ? lerp(0.5, NODE_Z + (inP5 ? p5 * 0.6 : 0), flyIn)
          : lerp(0, 0.5, p2);
        const segScale = inP4 ? lerp(1, 0.6, flyIn) : 1;
      ...
      {Array.from({ length: NUM_SEGS }, (_, s) => {
        ...
        return (
          <RenderNode
            ...
            edgeOpacity={
              inP4 ? 0.5 + processing * 0.4 : inP3 ? nodeFade * 0.5 : 0
            }
      ...
      <ProgressBar
        ...
        fillColor={COL_SEQ_FILL}
        emitColor={COL_SEQ_FILL}
      ...
      {Array.from({ length: NUM_SEGS }, (_, s) => {
        ...
        const barProg = clamp01(
          processing + barJitter[s]! * Math.sin(processing * Math.PI),
        );
        return (
          <ProgressBar
            key={s}
      ...
      <Particles
        timeMs={timeMs}
        visible={inP4 && processing > 0 && processing < 1}
      />
      ...
      <CompleteBlock
        opacity={
          parDone
            ? easeOut(prog(timeMs, P4_PAR_DONE, P4_PAR_DONE + 400)) * 0.95
            : 0
        }
        emissive={
          parDone
            ? inP5
              ? 0.6 + Math.sin(p5 * Math.PI * 3) * 0.2
              : easeOut(prog(timeMs, P4_PAR_DONE, P4_PAR_DONE + 400)) * 0.6
            : 0
        }
        scaleY={
          parDone
            ? lerp(
                0.2,
                1,
                easeOut(prog(timeMs, P4_PAR_DONE, P4_PAR_DONE + 400)),
              )
            : 0.2
        }
...
export function ParallelFragmentsCanvas({
  children,
}: {
  children?: ReactNode;
}) {
      ...
      >
        <Canvas
          ...
          scene={{
            ...
            fog: new THREE.Fog(0x1e2233, 16, 35),
          }}
