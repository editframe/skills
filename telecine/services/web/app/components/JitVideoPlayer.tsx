...
export default function JitVideoPlayer({ sourceUrl, className = '' }: JitVideoPlayerProps) {
  const videoRef = useRef<HTMLVideoElement>(null);
  const [manifest, setManifest] = useState<JitManifest | null>(null);
  const [currentRendition, setCurrentRendition] = useState<VideoRendition | null>(null);
  ...
  const loadManifest = useCallback(async () => {
    try {
      const response = await fetch(`/api/v1/transcode/manifest.json?url=${encodeURIComponent(sourceUrl)}`);
      if (!response.ok) {
      ...
      const mediumRendition = manifestData.videoRenditions.find(r => r.id === 'medium') || manifestData.videoRenditions[0];
      if (mediumRendition) {
  ...
  const calculateOptimalQuality = useCallback((recentTimings: SegmentTiming[]): VideoRendition | null => {
    if (!manifest || !currentRendition || recentTimings.length < 2) return currentRendition;

    // Calculate average transcoding time (from server headers - this is NOT user bandwidth)
    const avgTranscodeTime = recentTimings.reduce((sum, t) => sum + t.transcodeTime, 0) / recentTimings.length;

    // Calculate average server processing time (from server headers)
    const avgServerTime = recentTimings.reduce((sum, t) => sum + t.serverTime, 0) / recentTimings.length;

    // Calculate actual network transfer time (this accurately reflects user bandwidth)
    const avgDownloadTime = recentTimings.reduce((sum, t) => sum + t.downloadTime, 0) / recentTimings.length;

    // Estimate user bandwidth based on pure network transfer time
    const segmentSizeBytes = (currentRendition.bitrate * SEGMENT_DURATION_MS) / 8000; // Convert to bytes
    const currentBandwidth = avgDownloadTime > 0
    ...
    const filteredRenditions = manifest.videoRenditions.filter(r => r.bitrate <= availableBandwidth);
    const optimalRendition = filteredRenditions.length > 0
    ...
    if (optimalRendition) {
      console.log(`ABR Decision: transcode=${avgTranscodeTime.toFixed(0)}ms, server=${avgServerTime.toFixed(0)}ms, network=${avgDownloadTime.toFixed(0)}ms, bandwidth=${(currentBandwidth / 1000).toFixed(0)}kbps -> ${optimalRendition.id}`);
    }
  ...
  const loadVideoInitSegment = useCallback(async (rendition: VideoRendition) => {
    if (videoInitLoadedRef.current || !videoSourceBufferRef.current || !manifest) return;

    try {
      console.log(`Loading video initialization segment for ${rendition.id}...`);
      const initUrl = manifest.endpoints.initSegment.replace('{rendition}', rendition.id);
      const response = await fetch(initUrl);

      if (!response.ok) {
        throw new Error(`Video init segment fetch failed: ${response.status}`);
      }

      const initData = await response.arrayBuffer();
      console.log(`Video init segment fetched: ${initData.byteLength} bytes`);

      if (videoSourceBufferRef.current && !videoSourceBufferRef.current.updating &&
        ...
        await new Promise((resolve, reject) => {
          const onUpdate = () => {
            videoSourceBufferRef.current?.removeEventListener('updateend', onUpdate);
            videoSourceBufferRef.current?.removeEventListener('error', onError);
            videoInitLoadedRef.current = true;
            console.log('✅ Video initialization segment loaded successfully');
            resolve(void 0);
          };
          const onError = (e: any) => {
            videoSourceBufferRef.current?.removeEventListener('updateend', onUpdate);
            ...
            reject(new Error(`Video init segment append failed: ${e.type || 'Unknown error'}`));
          };
          videoSourceBufferRef.current?.addEventListener('updateend', onUpdate);
  ...
  const loadAudioInitSegment = useCallback(async (rendition: VideoRendition) => {
    if (audioInitLoadedRef.current || !audioSourceBufferRef.current || !manifest) return;

    try {
      console.log(`Loading audio initialization segment for ${rendition.id}...`);
      // Use 'audio' rendition for audio init segments
      const initUrl = manifest.endpoints.initSegment.replace('{rendition}', 'audio');
      const response = await fetch(initUrl);

      if (!response.ok) {
        throw new Error(`Audio init segment fetch failed: ${response.status}`);
      }
      ...
      if (audioSourceBufferRef.current && !audioSourceBufferRef.current.updating &&
        ...
        await new Promise((resolve, reject) => {
          const onUpdate = () => {
            audioSourceBufferRef.current?.removeEventListener('updateend', onUpdate);
            audioSourceBufferRef.current?.removeEventListener('error', onError);
            audioInitLoadedRef.current = true;
            console.log('✅ Audio initialization segment loaded successfully');
            resolve(void 0);
          };
          const onError = (e: any) => {
            audioSourceBufferRef.current?.removeEventListener('updateend', onUpdate);
            ...
            reject(new Error(`Audio init segment append failed: ${e.type || 'Unknown error'}`));
          };
          audioSourceBufferRef.current?.addEventListener('updateend', onUpdate);
  ...
  const fetchVideoSegmentInParallel = useCallback(async (segmentIndex: number, rendition: VideoRendition) => {
    ...
    try {
      ...
      const serverTranscodeTime = parseInt(response.headers.get('X-Transcode-Time-Ms') || '0', 10);
      const serverTotalTime = parseInt(response.headers.get('X-Total-Server-Time-Ms') || '0', 10);
      ...
      const networkTransferTime = Math.max(0, totalRequestTime - serverTotalTime);
  ...
  const fetchAudioSegmentInParallel = useCallback(async (segmentIndex: number) => {
    ...
    try {
      ...
      console.log(`🎵 Fetching audio segment ${segmentIndex} from: ${segmentUrl}`);
      const response = await fetch(segmentUrl);

      if (!response.ok) {
        console.error(`❌ Audio segment ${segmentIndex} HTTP error: ${response.status} ${response.statusText}`);
        throw new Error(`Audio segment fetch failed: ${response.status} ${response.statusText}`);
      }

      const segmentData = await response.arrayBuffer();
      const downloadEndTime = performance.now();

      console.log(`✅ Audio segment ${segmentIndex} fetched: ${segmentData.byteLength} bytes in ${(downloadEndTime - startTime).toFixed(0)}ms`);
      ...
      console.error(`❌ Failed to fetch audio segment ${segmentIndex}:`, error);
  ...
  const bufferAheadStrategy = useCallback(async () => {
    if (!mediaSourceRef.current || !videoSourceBufferRef.current || !audioSourceBufferRef.current || !manifest || !currentRendition ||
      videoSourceBufferRef.current.updating || audioSourceBufferRef.current.updating || mediaSourceRef.current.readyState !== 'open') {
      return;
    }
    ...
    if (!videoInitLoadedRef.current || !audioInitLoadedRef.current) {
      try {
        ...
        console.error('Failed to load init segments, stopping buffering:', error);
        return;
      }
    }
    ...
    console.log(`🔄 Buffer status: current=${currentTime.toFixed(1)}s, buffered=${bufferedEnd.toFixed(1)}s, target=${targetBufferEnd.toFixed(1)}s`);
    ...
    if (recentTimings.length >= 2) {
      ...
      if (newQuality && currentRendition && newQuality.id !== currentRendition.id) {
        console.log(`Quality change detected: ${currentRendition.id} -> ${newQuality.id}`);
        console.log('⚠️  Quality changes require MediaSource restart - skipping for now');
    ...
    const currentPositionBuffered = bufferedRanges.some(range =>
    ...
    const startSegment = Math.floor(bufferStartTime / (SEGMENT_DURATION_MS / 1000)) + 1;

    console.log(`📍 Current position ${currentTime.toFixed(1)}s is ${currentPositionBuffered ? 'buffered' : 'NOT buffered'} - starting from ${bufferStartTime.toFixed(1)}s`);

    // Always request a reasonable number of segments ahead (not huge gaps)
    const segmentsAhead = Math.min(
      PARALLEL_SEGMENT_COUNT,
      Math.ceil((targetBufferEnd - bufferStartTime) / (SEGMENT_DURATION_MS / 1000))
    );

    // Request segments starting from where we need them
    const segmentIndices = Array.from({ length: segmentsAhead }, (_, i) => startSegment + i);

    console.log(`📊 Buffering plan: start=${startSegment}, count=${segmentsAhead}, segments=[${segmentIndices.join(', ')}]`);
    console.log(`📊 Pending segments before request: ${Array.from(pendingSegmentsRef.current).join(', ')}`);

    // Set MediaSource duration if not already set
    if (mediaSourceRef.current && mediaSourceRef.current.duration !== manifest.duration) {
      try {
    ...
    console.log(`🚀 Starting parallel fetch for ${segmentIndices.length} segments`);

    // Helper function to append a segment as soon as it's ready
    const appendSegmentWhenReady = async (segmentIndex: number, segmentPromise: Promise<ArrayBuffer | null | undefined>, isAudio: boolean) => {
      try {
        const segmentData = await segmentPromise;
        if (!segmentData) {
          console.warn(`⚠️ ${isAudio ? 'Audio' : 'Video'} segment ${segmentIndex} returned null`);
          return;
        }

        const sourceBuffer = isAudio ? audioSourceBufferRef.current : videoSourceBufferRef.current;
        if (!sourceBuffer || sourceBuffer.updating || !mediaSourceRef.current || mediaSourceRef.current.readyState !== 'open') {
          console.warn(`⚠️ ${isAudio ? 'Audio' : 'Video'} SourceBuffer not ready for segment ${segmentIndex}`);
          return;
        }

        console.log(`⚡ ${isAudio ? 'Audio' : 'Video'} segment ${segmentIndex} ready - appending immediately (${segmentData.byteLength} bytes)`);
        ...
        await new Promise((resolve, reject) => {
          const onUpdate = () => {
            ...
            console.log(`✅ ${isAudio ? 'Audio' : 'Video'} segment ${segmentIndex} appended successfully`);
            ...
            setTimeout(() => {
              ...
              if (pendingCount === 0) {
                console.log(`🔄 Triggering immediate buffer check after ${isAudio ? 'audio' : 'video'} segment ${segmentIndex} completed (no pending segments)`);
                bufferStrategyRef.current?.();
              } else {
                console.log(`⏸️ Skipping immediate buffer check after ${isAudio ? 'audio' : 'video'} segment ${segmentIndex} - ${pendingCount} segments still pending`);
              }
          ...
          const onError = (e: any) => {
            sourceBuffer?.removeEventListener('updateend', onUpdate);
            sourceBuffer?.removeEventListener('error', onError);
            console.error(`❌ ${isAudio ? 'Audio' : 'Video'} segment ${segmentIndex} append failed:`, e);
            reject(e);
          };
        ...
        console.error(`❌ Failed to append ${isAudio ? 'audio' : 'video'} segment ${segmentIndex}:`, error);
      }
    };
    ...
    for (const segmentIndex of segmentIndices) {
      const videoPromise = fetchVideoSegmentInParallel(segmentIndex, currentRendition);
      const audioPromise = fetchAudioSegmentInParallel(segmentIndex);

      // Process each segment as soon as it's ready
      appendPromises.push(appendSegmentWhenReady(segmentIndex, videoPromise, false));
      appendPromises.push(appendSegmentWhenReady(segmentIndex, audioPromise, true));
    }
  ...
  }, [currentTime, bufferedRanges, segmentTimings, manifest, currentRendition, calculateOptimalQuality, fetchVideoSegmentInParallel, fetchAudioSegmentInParallel, loadVideoInitSegment, loadAudioInitSegment]);
  ...
  useEffect(() => {
    ...
    const handleSourceOpen = () => {
      try {
        ...
        console.log(`Video: ${videoCodec} (supported: ${MediaSource.isTypeSupported(videoCodec)})`);
        console.log(`Audio: ${audioCodec} (supported: ${MediaSource.isTypeSupported(audioCodec)})`);

        if (!MediaSource.isTypeSupported(videoCodec) || !MediaSource.isTypeSupported(audioCodec)) {
  ...
  useEffect(() => {
    ...
    const handleSeeked = async () => {
      ...
      console.log(`🎯 Seek completed to ${seekTime.toFixed(1)}s - clearing buffers and restarting`);
      ...
      try {
        ...
        if (videoSourceBufferRef.current && !videoSourceBufferRef.current.updating) {
          ...
          for (let i = 0; i < buffered.length; i++) {
            ...
            console.log(`🗑️ Removing video buffer range ${start.toFixed(1)}-${end.toFixed(1)}s`);
        ...
        if (audioSourceBufferRef.current && !audioSourceBufferRef.current.updating) {
          ...
          for (let i = 0; i < buffered.length; i++) {
            const start = buffered.start(i);
            const end = buffered.end(i);
            console.log(`🗑️ Removing audio buffer range ${start.toFixed(1)}-${end.toFixed(1)}s`);
        ...
        setTimeout(() => {
          console.log(`🎬 Starting buffering from seek position ${seekTime.toFixed(1)}s`);
          bufferStrategyRef.current?.();
        }, 100);
  ...
  useEffect(() => {
    if (!videoSourceBufferRef.current || !audioSourceBufferRef.current || !mediaSourceRef.current || !manifest || !currentRendition) {
      return;
    }

    if (mediaSourceRef.current.readyState === 'open' && !mediaSourceReadyRef.current) {
      mediaSourceReadyRef.current = true;
      console.log('🎬 MediaSource ready for first time, starting initial buffering');
  ...
  useEffect(() => {
    ...
    const bufferInterval = setInterval(() => {
      if (videoSourceBufferRef.current && audioSourceBufferRef.current && mediaSourceRef.current?.readyState === 'open') {
        // Update currentTime and bufferedRanges directly from video element
        const video = videoRef.current;
        if (video) {
          const newCurrentTime = video.currentTime;
          console.log(`⏰ Interval update: currentTime=${newCurrentTime.toFixed(1)}s, duration=${video.duration?.toFixed(1)}s`);
  ...
  return (
    <div className={`relative ${className}`}>
      ...
      <div className="mt-2 text-xs text-gray-600 space-y-1">
        <div>Quality: {currentRendition?.id} ({currentRendition?.width}x{currentRendition?.height})</div>
        <div>Video init: {videoInitLoadedRef.current ? '✓ Loaded' : '⏳ Loading'}</div>
        <div>Audio init: {audioInitLoadedRef.current ? '✓ Loaded' : '⏳ Loading'}</div>
        <div>Buffered: {bufferedRanges.map(r => `${r.start.toFixed(1)}-${r.end.toFixed(1)}`).join(', ')}</div>
        <div>Parallel segments: {pendingSegmentsRef.current.size}</div>
        <div>Recent timings: {segmentTimings.slice(-3).map(t => `${t.totalTime.toFixed(0)}ms`).join(', ')}</div>
      </div>
    </div>
  );
}