...
function calculateAudioSegmentDurationsInternal(
  ...
  while (segmentIndex * targetSegmentDurationMs < totalDurationMs) {
    ...
    const durationMs = truncateDecimal(duration / 1000, 5);
...
function calculateVideoSegmentDurationsInternal(
  ...
  for (let i = 0; i < totalSegments; i++) {
    ...
    const actualFrames = targetFrames - 2; // 48 frames actually produced
    const actualDurationMs = actualFrames * frameMs; // 48 * 40 = 1920ms
