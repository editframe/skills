...
export function parseAttributeFiltersFromUrl(searchParams: URLSearchParams): AttributeFilter[] {
...
function parseFiltersFromUrl(searchParams: URLSearchParams, prefix: string): Map<string, FilterState> {
  const filters = new Map<string, FilterState>();

  const showParam = searchParams.get(`${prefix}Show`);
  if (showParam) {
    showParam.split(',').filter(Boolean).forEach(name => filters.set(name, 'show'));
  }

  const hideParam = searchParams.get(`${prefix}Hide`);
  if (hideParam) {
    hideParam.split(',').filter(Boolean).forEach(name => filters.set(name, 'hide'));
  }
...
export function FilterSidebar({ traces, onFilterChange, searchParams, setSearchParams, attributeFilters }: FilterSidebarProps) {
  ...
  const [serviceFilters, setServiceFilters] = useState<Map<string, FilterState>>(() =>
  ...
  const filteredTraceNames = useMemo(() => {
    ...
    return traceNames.filter(name => {
      const hasFilter = traceFilters.has(name) && traceFilters.get(name) !== 'neutral';
      const matchesSearch = name.toLowerCase().includes(traceSearch.toLowerCase());
  ...
  const filteredSpanNames = useMemo(() => {
    ...
    return spanNames.filter(name => {
      const hasFilter = spanFilters.has(name) && spanFilters.get(name) !== 'neutral';
      const matchesSearch = name.toLowerCase().includes(spanSearch.toLowerCase());
      return hasFilter || matchesSearch;
    });
  ...
  const filteredServiceNames = useMemo(() => {
    ...
    return serviceNames.filter(name => {
      const hasFilter = serviceFilters.has(name) && serviceFilters.get(name) !== 'neutral';
      const matchesSearch = name.toLowerCase().includes(serviceSearch.toLowerCase());
      return hasFilter || matchesSearch;
    });
  ...
  const handleTraceSearchKeyDown = (e: React.KeyboardEvent) => {
    ...
    } else if ((e.key === 'Enter' || e.key === ' ') && traceSelectedIndex >= 0) {
      e.preventDefault();
  ...
  const handleServiceSearchKeyDown = (e: React.KeyboardEvent) => {
    ...
    } else if ((e.key === 'Enter' || e.key === ' ') && serviceSelectedIndex >= 0) {
      e.preventDefault();
  ...
  const filteredTraces = useMemo(() => {
    ...
    const hasShowFilters = showTraces.length > 0 || showSpans.length > 0 || showServices.length > 0;
    ...
    return traces.filter((trace) => {
      ...
      const hasHiddenSpan = spanNamesInTrace.some((name) => hideSpans.includes(name));
      if (hasHiddenSpan) {
        return false;
      }

      const hasHiddenService = serviceNamesInTrace.some((name) => hideServices.includes(name));
      if (hasHiddenService) {
        return false;
      }
      ...
      for (const filter of hideAttrFilters) {
        const matchingSpans = trace.allSpans.filter(s => s.name === filter.spanName);
        ...
        const hasMatchingSpan = matchingSpans.some(span => {
          const attr = span.attributes.find(a => a.key === filter.attributeKey);

          switch (filter.condition) {
            case 'exists':
              return attr !== undefined && attr.value !== null && attr.value !== undefined;
            case 'missing':
              return attr === undefined || attr.value === null || attr.value === undefined;
            case 'equals':
      ...
      if (hasShowFilters) {
        ...
        const matchesSpanShow = spanNamesInTrace.some((name) => showSpans.includes(name));
        const matchesServiceShow = serviceNamesInTrace.some((name) => showServices.includes(name));
  ...
  return (
    <div className="filter-sidebar">
      <div className="filter-section">
        <div className="filter-section-header">Traces ({traceNames.length})</div>
      ...
      <div className="filter-section">
        <div className="filter-section-header">Services ({serviceNames.length})</div>
        <div className="filter-search-container">
          <input
            type="text"
