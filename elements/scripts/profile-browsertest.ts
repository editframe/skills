...
`);
  ...
  if (totalSamples > 0) {
    ...
    const reportPath = await generateMarkdownReport(analysisData, testFile, testPattern, profilesDir);
...
async function printProfileAnalysis(profile: CPUProfile, focusFile: string, wallClockMs: number) {
  ...
  console.log(`  Total Samples: ${totalSamples}`);
  console.log(`  Sampling Interval: ${sampleIntervalUs}μs (${(sampleIntervalUs / 1000).toFixed(1)}ms)`);
  ...
  console.log(`├──────┼────────────┼─────────┼─────────┼───────────────────────────────────┤`);
  const topFilesBySelf = Array.from(byFileSelf.entries()).sort((a, b) => b[1] - a[1]).slice(0, 20);
  ...
  if (ourCode.length > 0) {
    ...
    bySelf.forEach((h, idx) => {
      ...
      const locationStr = location.length > 40 ? location.slice(0, 37) + '...' : location.padEnd(40);
    ...
    byTotal.forEach((h, idx) => {
      ...
      const location = `${h.functionName} @ ${h.file}:${h.line}`;
      const locationStr = location.length > 39 ? location.slice(0, 36) + '...' : location.padEnd(39);
    ...
    if (bySelf.length > 0) {
      ...
      console.log(`│ ${topHotspot.functionName} @ ${topHotspot.file}:${topHotspot.line}`.padEnd(79) + '│');
      console.log(`│ Self: ${topHotspot.selfTimeMs.toFixed(1)}ms | Total: ${topHotspot.totalTimeMs.toFixed(1)}ms`.padEnd(79) + '│');
      console.log(`├────────────────────────────────────────────────────────────────────────────┤`);
    ...
    byFrequency.forEach((h, idx) => {
      ...
      const samplesStr = h.selfSamples.toString().padStart(7);
      const location = `${h.functionName} @ ${h.file}:${h.line}`;
      const locationStr = location.length > 58 ? location.slice(0, 55) + '...' : location.padEnd(58);
  ...
  return {
    ...
    totalSamples,
    sampleIntervalUs,
    topFilesBySelf: Array.from(byFileSelf.entries()).sort((a, b) => b[1] - a[1]).slice(0, 20),
    ourCode,
    hotspots,
  };
}
...
async function generateMarkdownReport(
  ...
  const bySelf = [...analysis.ourCode].sort((a: any, b: any) => b.selfTimeMs - a.selfTimeMs).slice(0, 20);
  const byTotal = [...analysis.ourCode].sort((a: any, b: any) => b.totalTimeMs - a.totalTimeMs).slice(0, 20);
  const byFrequency = [...analysis.ourCode].sort((a: any, b: any) => b.selfSamples - a.selfSamples).slice(0, 20);
  ...
  analysis.topFilesBySelf.forEach(([file, time]: [string, number], idx: number) => {
    ...
    const samples = analysis.hotspots.filter((h: any) => h.file === file).reduce((sum: number, h: any) => sum + h.selfSamples, 0);
  ...
  const tsFiles = analysis.ourCode.filter((h: any) => h.file.endsWith('.ts') || h.file.endsWith('.tsx'));
  if (tsFiles.length > 0) {
