...
import {
  UnifiedByteRangeFetcher,
  validateByteRangeSupport,
} from "../src/async/UnifiedByteRangeFetcher.js";
...
async function demonstrateLocalFileSupport() {
  ...
  for (const url of examples.slice(0, 3)) {
    // Skip HTTP for this demo
    try {
      ...
      console.log(
        `   ${result.error ? `└─ Error: ${result.error}` : "└─ ✅ Ready"}\n`,
      );
    } catch (error) {
      console.log(`   ${url}`);
      console.log(
