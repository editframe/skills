...
import type { Video2UnprocessedFiles } from "@/sql-client.server/kysely-codegen";

import type { Route } from "./+types/delete_unprocessed_files";
import { requireActionSecretOrThrow } from "@/http/requireActionSecret";

export const action = async ({ request }: Route.ActionArgs) => {
  requireActionSecretOrThrow(request);
  return executeSpan("delete_unprocessed_files", async (span) => {
    const payload = await request.json() as HasuraEvent<Selectable<Video2UnprocessedFiles>>;
    span.setAttributes({
