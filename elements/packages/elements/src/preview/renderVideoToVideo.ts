 ...
 * 
 * Supports CSS filter and opacity via canvas 2D context.
 * Audio uses the existing renderTemporalAudio() pipeline.
 */
...
export async function renderVideoToVideo(
  ...
  try {
    for (let frameIndex = 0; frameIndex < config.totalFrames; frameIndex++) {
      ...
      try {
        // Draw to encoding canvas with CSS effects
        ...
        // Reset context state
        if (hasFilter) {
          encodingCtx.filter = "none";
        }
        if (hasOpacity) {
          encodingCtx.globalAlpha = 1;
        }
