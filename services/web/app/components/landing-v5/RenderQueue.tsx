...
export function useRenderQueue(): RenderQueueContextValue {
  ...
  if (!ctx)
    throw new Error("useRenderQueue must be used within RenderQueueProvider");
  return ctx;
}
  ...
  useEffect(() => {
    ...
    if (!tg?.renderToVideo) {
      updateJob(nextQueued.id, {
        status: "error",
        error:
          "Element does not support rendering. Expected an <ef-timegroup> or <ef-video> element.",
      });
    ...
    const flushProgress = () => {
      ...
      if (pendingPatch) {
        if (pendingPreviewCanvas) {
          pendingPatch.previewFrame = pendingPreviewCanvas.toDataURL(
            "image/jpeg",
            0.5,
          );
      ...
      onProgress: (p: {
        ...
        if (now - lastProgressFlush >= PROGRESS_THROTTLE_MS) {
          ...
          flushTimer = setTimeout(
            flushProgress,
            PROGRESS_THROTTLE_MS - (now - lastProgressFlush),
          );
        }
      },
      ...internal.renderOpts,
...
export function RenderQueuePanel() {
  ...
  const activeCount = jobs.filter(
    (j) => j.status === "rendering" || j.status === "queued",
  ).length;
      ...
      {!minimized && (
        <div className="border-t border-white/10">
          {jobs.map((job) => (
            <div
              key={job.id}
              className={`px-3 py-2.5 border-b border-white/5 last:border-b-0 ${job.status === "complete" ? "bg-[#4CAF50]/10" : ""}`}
            >
              {/* ── Queued ─────────────────────────────────────────── */}
              {job.status === "queued" && (
                <>
                  ...
                  <div className="text-[9px] text-white/30 truncate mb-1">
                    {job.fileName}
                  </div>
              ...
              {job.status === "rendering" && (
                <>
                  <div className="flex items-center justify-between mb-1">
                    ...
                    {job.progress >= 0.99 ? (
                      <span className="text-[10px] text-[var(--poster-gold)] font-semibold animate-pulse">
                        Finishing...
                      </span>
                    ) : (
                    ...
                    <div className="grid grid-cols-2 gap-x-3 gap-y-1 mb-1.5 font-mono text-[10px] tabular-nums">
                      <div>
                        ...
                        <div className="text-white/70">
                          {job.stats.currentFrame} / {job.stats.totalFrames}
                        </div>
                      </div>
                      <div>
                        <div className="text-white/30">Time</div>
                        <div className="text-white/70">
                          {formatTime(job.stats.renderedMs)} /{" "}
                          {formatTime(job.stats.totalDurationMs)}
                        </div>
                      </div>
                      <div>
                      ...
                      <div>
                        ...
                        <div className="text-white/70">
                          {formatTime(job.stats.estimatedRemainingMs)}
                        </div>
                      </div>
                    </div>
                  )}
              ...
              {job.status === "complete" && (
                <>
                  <div className="flex items-center gap-2 mb-2">
                    <svg
                      className="w-4 h-4 text-[#4CAF50] flex-shrink-0"
                      fill="currentColor"
                      viewBox="0 0 24 24"
                    >
                      <path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm-2 15l-5-5 1.41-1.41L10 14.17l7.59-7.59L19 8l-9 9z" />
                    </svg>
                    <span className="text-[11px] font-semibold text-[#4CAF50] truncate flex-1">
                      {job.name}
                    </span>
                    <button
              ...
              {job.status === "error" && (
                <>
                  <div className="flex items-center justify-between mb-1">
                    <span className="text-[11px] font-medium text-red-400 truncate flex-1 mr-2">
                      {job.name}
                    </span>
                    <button
                      onClick={() => remove(job.id)}
