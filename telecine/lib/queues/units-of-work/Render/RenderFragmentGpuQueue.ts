...
const MAX_WORKER_COUNT = envInt("RENDER_FRAGMENT_GPU_MAX_WORKER_COUNT", 1);
const WORKER_CONCURRENCY = envInt("RENDER_FRAGMENT_GPU_WORKER_CONCURRENCY", 8);
...
export const RenderFragmentGpuQueue = new Queue<{
    ...
    for (const msg of messages) {
      ...
      try {
        ...
        logger.error(
          ...
          "Failed to record GPU fragment start",
        );
      }
    }
  },
  ...
  processFailures: async (messages, db) => {
    for (const msg of messages) {
      ...
      try {
        ...
        logger.error(
          { jobId: msg.jobId, workflowId: msg.workflowId, segmentId, error },
          "Failed to record GPU fragment failure",
        );
      }
    }
  },
  ...
  processCompletions: async (messages, db) => {
    for (const msg of messages) {
      ...
      try {
        ...
        logger.error(
          { jobId: msg.jobId, workflowId: msg.workflowId, segmentId, error },
          "Failed to record GPU fragment completion",
        );
      }
    }
  },
