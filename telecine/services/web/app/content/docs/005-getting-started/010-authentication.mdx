...
You will need your API token which can be found in the [Editframe dashboard](https://editframe.com/resource/api_keys).
...
```bash
curl https://editframe.com/api/v1/[...] \
...
**Important:** URL signing must happen on your server, not in the client. Here's the flow:

1. **Server-side:** Your server uses your `EF_API_TOKEN` to create a signed token for a specific URL
2. **Server-to-client:** Your server passes this signed token to your client application
3. **Client-side:** The client uses the signed token in the browser to make authenticated requests

Here's how to implement URL signing on your server:
...
// This runs on your server - keep EF_API_TOKEN secret!
const EF_TOKEN = process.env.EF_API_TOKEN;
const client = new Client(EF_TOKEN);

const url = "https://editframe.com/api/v1/unprocessed_files";
const token = await createURLToken(client, url);

// Pass this token to your client (e.g., via API response)
return { token };
```

On the client side, you would then use this token:

```ts
// Client-side code
const response = await fetch('/api/get-signed-token'); // Your server endpoint
const { token } = await response.json();

// Use the token to make authenticated requests
await fetch("https://editframe.com/api/v1/unprocessed_files", {
  headers: {
