...
const filters: Filter[] = [
  ...
  },
  {
    ...
    cssFilter:
      "hue-rotate(15deg) saturate(1.2) brightness(1.05) contrast(1.05)",
  },
  {
    id: "warm",
    name: "Warm",
    ...
    cssFilter:
      "hue-rotate(-15deg) saturate(1.3) brightness(1.1) contrast(0.95)",
  },
  {
    id: "dramatic",
    name: "Dramatic",
...
function manualValuesToFilter(
  values: ManualFilterValues,
  filterId: string,
): { css: string; needsSvg: boolean } {
  const cssFilters = [];
  const needsSvgEffects = values.sharpen > 0 || values.vignette > 0;

  if (values.brightness !== 100)
    cssFilters.push(`brightness(${values.brightness / 100})`);
  if (values.contrast !== 100)
    cssFilters.push(`contrast(${values.contrast / 100})`);
  if (values.saturate !== 100)
    cssFilters.push(`saturate(${values.saturate / 100})`);
  if (values.hueRotate !== 0)
    cssFilters.push(`hue-rotate(${values.hueRotate}deg)`);
  if (values.sepia !== 0) cssFilters.push(`sepia(${values.sepia / 100})`);
  if (values.grayscale !== 0)
    cssFilters.push(`grayscale(${values.grayscale / 100})`);
  if (values.blur !== 0) cssFilters.push(`blur(${values.blur}px)`);
  if (values.opacity !== 100)
    cssFilters.push(`opacity(${values.opacity / 100})`);
...
export default function Filters(_props: Route.ComponentProps) {
  ...
  const updateManualValue =
    (key: keyof ManualFilterValues) => (value: number) => {
      setManualValues((prev) => ({ ...prev, [key]: value }));
    };

  const manualFilter = manualValuesToFilter(manualValues, "manual-filter");
  const activeSvgFilterDef = manualFilter.needsSvg
    ? generateSvgFilter(manualValues, "manual-filter")
    : undefined;

  const activeFilterCss =
    filterMode === "presets" ? selectedFilter.cssFilter : manualFilter.css;
      ...
      <div className="grid grid-cols-[360px_1fr] gap-1 min-h-0 overflow-hidden h-full">
              ...
              <div className="space-y-2">
                ...
                <div className="grid grid-cols-3 gap-2">
                  {filters.map((filter) => (
                    ...
                    >
                      <div
                        className={`w-full aspect-square rounded border-2 overflow-hidden mb-1 transition-colors ${
                          selectedFilter.id === filter.id
                            ? "border-blue-500"
        ...
        <section className="bg-gradient-to-br from-black to-gray-900 rounded-lg border border-gray-700 flex flex-col">
          ...
          <div className="flex-1 flex items-center justify-center overflow-hidden">
            <Timegroup mode="contain" className="relative w-full h-auto">
              <Video
                id="filter-video"
