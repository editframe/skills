...
async function main() {
  ...
  try {
    ...
    const seekEvents = await page.evaluate(async ({ duration, scrubSpeed, totalDuration }) => {
      const timegroup = document.querySelector("ef-timegroup") as any;
      if (!timegroup) {
        throw new Error("Timegroup not found");
      }
      
      const seekEvents: Array<{ timestamp: number; seekTimeMs: number; videoDesiredSeekTimes: number[] }> = [];
    ...
    if (seekEvents.length > 0 && sortedFrames.length > 0) {
      ...
      console.log(`   First frame: ${sortedFrames[0].videoFrameTimestampMs}ms at ${sortedFrames[0].timestamp}ms`);
    }
    ...
    for (let i = 0; i < seekEvents.length; i++) {
      ...
      let bestMatch: { frame: any; index: number; timeDiff: number; timestampDiff: number } | null = null;
    ...
    console.log(`     Displayed: ${framesDisplayed} (${((framesDisplayed / matchedEvents.length) * 100).toFixed(1)}%)`);
    console.log(`     Dropped: ${framesDropped} (${((framesDropped / matchedEvents.length) * 100).toFixed(1)}%)`);
    console.log(`     Total frames rendered: ${finalFrameEvents.length} (may include frames from multiple videos)`);
    ...
    if (matchedEvents.length > 0 && finalFrameEvents.length > 0) {
      ...
      for (let i = 0; i < Math.min(10, matchedEvents.length); i++) {
        ...
        if (event.frameDisplayed) {
          console.log(`     ${timeSinceStart.toFixed(0)}ms: Seek ${event.seekTimeMs.toFixed(0)}ms → Frame ${event.displayedTimeMs?.toFixed(0)}ms (latency: ${event.seekLatencyMs?.toFixed(0)}ms)`);
        } else {
          console.log(`     ${timeSinceStart.toFixed(0)}ms: Seek ${event.seekTimeMs.toFixed(0)}ms → DROPPED`);
        }
      }
    }
    ...
    const profile: ScrubProfile = {
      ...
      framesDisplayed,
      framesDropped,
      averageFrameIntervalMs: frameIntervals.length > 0 
      ...
      averageSeekLatencyMs: matchedEvents.filter(e => e.seekLatencyMs !== null).length > 0
        ? matchedEvents.filter(e => e.seekLatencyMs !== null)
            .reduce((sum, e) => sum + (e.seekLatencyMs || 0), 0) / framesDisplayed
        : 0,
      maxSeekLatencyMs: Math.max(...matchedEvents.filter(e => e.seekLatencyMs !== null).map(e => e.seekLatencyMs || 0)),
