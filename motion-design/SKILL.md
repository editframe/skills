...
Do not copy catalog example source. Those files use catalog helpers that a project does not include.
...
Preview a study at `https://editframe.com/motion#/{id}`.
...
| id | name | collection | scale | problem | typical |
|---|---|---|---|---|---|
| `social-ad-spine` | Social Ad Spine | — | composition | A social ad that is only a hero shot feels like a still; one that is only a logo feels like a bumper. The viewer needs a spine they can feel in one sitting. | 20s |
| `product-demo-documentary` | Product Demo Documentary | — | composition | A UI recreation without a camera plan is a screen recording. A camera plan without a documentary spine is a random zoom. | 20s |
| `launch-montage` | Launch Montage | — | composition | A launch that only shows UI feels small. A launch that only shows type feels empty. The film has to alternate proof and name. | 22s |
| `character-brand-film` | Character Brand Film | — | composition | A lockup that never performs is a still. A mascot performance that never becomes a wordmark is a cartoon with no brand. | 12s |
| `agent-workflow-reel` | Agent Workflow Reel | — | composition | Agent demos sprawl into 30s of identical typing. The viewer needs phases: brief, work, artifact — not a log. | 20s |
| `editorial-title-sequence` | Editorial Title Sequence | — | composition | One long title card has no progression; a montage of unrelated type styles feels like separate films. | 12s |
...
| `staggered-checklist-resolve` | Staggered Checklist Resolve | — | scene | A list of setup steps shown all at once feels finished before it is read; shown too slowly feels like a loader. | 2.5s |
| `monospace-terminal-stack` | Monospace Terminal Stack | — | scene | A CLI install that types in place does not feel like output accumulating. | 5s |
...
| `slot-machine-ticker` | Word Slot | Typography | scene | Crossfading successive words obscures their relationship and gives each state no physical continuity. | 6s |
...
| `tunnel-seal-rush` | Tunnel Seal Rush | Camera & depth | scene | An opener that fades from black has no velocity. A tunnel without sequenced objects is a screensaver. | 3.2s |
| `hexagon-2d-to-3d` | Hexagon 2D to 3D | Camera & depth | scene | Jumping from a diagram to an unrelated 3D object breaks visual continuity. | 6s |
...
| `word-split-title-reveal` | Letter Rearrangement | Typography | scene | A crossfade hides letter identity; straight crossing paths make the moving letters overlap. | 6s |
...
| `kinetic-type-poster` | Elastic Tracking | Typography | scene | An entrance animation does not demonstrate how changing the space between letters alters the rhythm of a word. | 6s |
| `type-outline-echo` | Outline Type Echo | Typography | scene | Repeated text becomes clutter when every copy has equal weight or unrelated motion. | 6s |
| `orbital-phase-lock` | Orbital Phase Lock | Shape & rhythm | scene | Independent moving elements look arbitrary unless the viewer can perceive the relationship between their periods. | 8s |
| `wave-interference-field` | Wave Interference Field | Shape & rhythm | scene | A dense line texture feels static when its complexity has no visible cause. | 8s |
| `glyph-foundry` | Glyph Foundry | Typography | scene | Letterforms should emerge from a visible construction system. | 7.2s |
| `baseline-orbit` | Baseline Orbit | Typography | scene | Type needs to travel along a changing curve without losing its spacing. | 8s |
| `contour-current` | Contour Current | Typography | scene | A word should become a moving material while remaining recognizable at key moments. | 7s |
| `letter-gait` | Letter Gait | Typography | scene | Lettering should perform an action through its own structure. | 5.4s |
| `axis-duet` | Axis Duet | Typography | scene | Letter proportions should change the relationship between two words. | 5.8s |
| `trace-memory` | Trace Memory | Typography | scene | Drawing a word should expose the order and character of its strokes. | 7s |
| `lower-third-anchor` | Lower Third Anchor | Layouts | scene | Identify a subject without obscuring the main action. | 7.8s |
| `split-panel-handoff` | Split Panel Handoff | Layouts | scene | Two subjects need to share the frame while their hierarchy changes. | 8.4s |
| `tracked-callout-overlay` | Tracked Callout Overlay | Layouts | scene | An annotation must remain attached to a moving subject without covering it. | 8s |
| `orbiting-sculpture` | Orbiting Sculpture | 3D | scene | A complex form cannot be understood from one viewpoint. | 10s |
| `hinged-cube-net` | Hinged Cube Net | 3D | scene | The relationship between a flat net and a solid should remain visible. | 9s |
| `metaball-coalescence` | Metaball Coalescence | Shape & rhythm | scene | Separate forms should merge into a continuous material. | 8s |
| `match-cut-relay` | Match Cut Relay | Transitions | scene | A cut between unrelated scenes should preserve the viewer’s visual focus. | 7.2s |
| `volumetric-wave` | Spatial Wave | 3D | scene | A timing wave should reveal the depth and continuity of a spatial field. | 8.4s |
| `orbital-armillary` | Orbital Armillary | 3D | scene | Orbital alignment should remain intelligible when paths occupy different planes. | 10s |
| `contour-slice-volume` | Contour Slice Volume | 3D | scene | A volume’s cross-sections should remain recognizable while its silhouette deforms. | 8.6s |
| `voxel-regrouping` | Voxel Regrouping | 3D | scene | A spatial quantity should change arrangement without changing its membership. | 9s |
...
| `progress-ring-fill` | Progress Ring Fill | Lines & data | part | Numbers alone make relative proportions hard to compare at a glance. | 5s |
...
| `type-as-window` | Chromatic Type Window | Typography | part | Animating the entire word undermines legibility; a static color fill can make large typography feel inert. | 8s |
...
| `inset-focus-relay` | Inset Focus Relay | Layouts | part | A detail needs emphasis without losing the larger scene. | 8.8s |
| `comparison-scan` | Comparison Scan | Layouts | part | A comparison should reveal differences without making the viewer mentally align two images. | 8s |
| `caption-safe-reflow` | Caption-safe Reflow | Layouts | part | Captions need a readable zone without covering the subject. | 8.4s |
| `impulse-relay` | Impulse Relay | Shape & rhythm | part | One movement should visibly cause the next rather than sharing a preset stagger. | 7.8s |
| `lagging-linkage` | Lagging Linkage | Shape & rhythm | part | Secondary motion should follow the leading action while connected parts remain joined. | 7.8s |
| `pressure-field` | Pressure Field | Shape & rhythm | part | A focal change should affect its surrounding composition through local relationships. | 9s |
| `focus-interface-typing` | Interface Typing | Focus & interaction | part | A typed label can read as a title reveal unless the viewer sees an editable field and a response. | 9s |
| `focus-cursor-path` | Cursor Path | Focus & interaction | part | A cursor crossing a busy interface is easy to lose; a permanent trail competes with its destination. | 7s |
| `focus-collaborative-cursors` | Collaborative Cursors | Focus & interaction | part | Multiple pointers become visual noise when ownership and contribution are unclear. | 9s |
| `focus-chat-turn-taking` | Chat Turn-Taking | Focus & interaction | part | Chat animation can confuse composition, sending, waiting and answering when all messages simply appear. | 12s |
| `focus-selective-blur` | Selective Blur | Focus & interaction | part | A focus effect becomes a transition gimmick when it obscures the target or moves the interface unnecessarily. | 8s |
| `caption-fade-up` | Caption Fade Up | — | motif | A caption that pops in place is a UI label. Rising 20–30px is a lower-third. | — |
| `weightless-bob` | Weightless Bob | Shape & rhythm | motif | A stationary object has no perceived weight; a uniform up-and-down tween feels mechanical. | 6s |
| `circular-bloom-wipe` | Circular Bloom Wipe | Transitions | motif | A cut separates two visual worlds. How can a small focal point become the next entire frame? | 5.6s |
| `horizontal-panel-wipe` | Horizontal Panel Wipe | Transitions | motif | A dissolve blurs the relationship between two graphic states. A moving seam can make the replacement legible. | 5.4s |
| `color-block-wipe` | Color Block Wipe | Transitions | motif | A transition can feel anonymous when it has no graphic relationship to the composition. | 5.2s |
...
| `typewriter` | Typewriter | Typography | motif | Uniform character timing feels mechanical and ignores the pauses between words and punctuation. | 5s |
...
| `ticker-marquee` | Counter-running Marquee | Typography | motif | A single scrolling line often feels like a news ticker; inconsistent speeds or visible seams break the illusion of endless motion. | 8s |
| `pixel-twinkle` | Pixel Twinkle | — | motif | Empty corners on a title card are dead. Seeded pixels that twinkle are a field. | — |
| `pixel-converge` | Pixel Converge | Shape & rhythm | motif | An identity or structure needs an arrival with a clear change from disorder to order. | 5s |
| `zoom-blast` | Zoom Blast | Camera & depth | motif | A fast camera transition should preserve a focal point and resolve into a readable destination. | 4.8s |
| `whip-pan` | Whip Pan | Transitions | motif | A simple cut disconnects adjacent scenes, but a slow camera move drains their momentum. | 5s |
| `scroll-flicks` | Scroll Flicks | Camera & depth | motif | A smooth scroll is a tween. Discrete flicks with pauses are a person. | 4s |
...
| `rule-draw` | Editorial Rule Draw | Lines & data | motif | A static divider can look arbitrary; drawing it before the title lands steals attention from the heading. | 4s |
| `editorial-wordmark-hook` | Editorial Masthead | — | motif | A wordmark that simply fades up has little character, while a spring or slam conflicts with a quiet editorial voice. | 5s |
| `shape-morph-loop` | Shape Morph Loop | Shape & rhythm | motif | Switching between disconnected shapes loses the sense that one form is changing its material state. | 6s |
| `caption-classic-subtitle` | Classic Subtitle | Typography | motif | Readers need a complete phrase that stays steady while the image continues. | 10s |
| `caption-karaoke-fill` | Karaoke Fill | Typography | motif | Readers need to follow progress inside a word without losing the rest of the line. | 10s |
| `caption-word-pop` | Word Pop | Typography | motif | A short spoken phrase needs a strong focal beat with each word. | 10s |
| `caption-paint-on` | Paint-on Caption | Typography | motif | A sentence should build with speech while leaving earlier words available to read. | 10s |
| `caption-transcript-rollup` | Transcript Roll-up | Typography | motif | Continuous narration needs recent context without an ever-growing transcript. | 10s |
| `manim-write-transform` | Manim Write & Transform | Typography | motif | Changing an equation can lose the identity of its terms and obscure the reasoning. | 12.5s |
| `foreground-object-wipe` | Foreground Object Wipe | Transitions | motif | A scene change needs to be motivated by something moving inside the composition. | 8s |
| `luma-contour-reveal` | Luma Contour Reveal | Transitions | motif | A reveal needs spatial ordering beyond a straight traveling edge. | 8s |
| `liquid-scene-flow` | Liquid Scene Flow | Transitions | motif | Organic scenes need a transition whose boundary participates in their movement. | 8s |
| `paper-tear-reveal` | Paper Tear Reveal | Transitions | motif | Editorial artwork needs a tactile scene handoff. | 8s |
| `pixel-scene-replacement` | Pixel Scene Replacement | Transitions | motif | Digital scenes need discrete replacement rather than another object assembly. | 8s |
| `card-flip-transition` | Card Flip Transition | Transitions | motif | A mosaic needs to reveal a different image through surface turnover. | 8s |
| `band-smear-handoff` | Band Smear Handoff | Transitions | motif | A graphic scene change needs to carry directional distortion through the edit. | 8s |
| `displacement-handoff` | Displacement Handoff | Transitions | motif | A material scene change needs a continuous distortion rather than a flat cover. | 8s |

### Aesthetics

| id | name | tag |
|---|---|---|
| `aesthetic-vaporwave` | Vaporwave | A place in memory. |
| `aesthetic-goth` | Goth | Beauty in the shadows. |
| `aesthetic-edwardian` | Edwardian | Most cordially invited. |
| `aesthetic-beach` | Beach | Nothing on your calendar. |
| `aesthetic-stone` | Stone | The grain of time. |
| `aesthetic-glass` | Glass | Stronger in layers. |
| `aesthetic-swiss-editorial` | Swiss Editorial | Order, with conviction. |
| `aesthetic-memphis-play` | Memphis Play | A little more, please. |
| `aesthetic-scientific-instrument` | Scientific Instrument | Make the invisible legible. |
| `aesthetic-liquid-organic` | Liquid Organic | Nothing moves alone. |
| `aesthetic-retro-computing` | Retro Computing | A system coming to life. |
| `aesthetic-optical-rhythm` | Optical Rhythm | Look, then look again. |

### Timing

| id | name | use |
|---|---|---|
| `timing-linear` | Linear | Make distance and elapsed time easy to compare. |
| `timing-slow` | Slow | Let a transformation feel deliberate and easy to follow. |
| `timing-fast` | Fast | Create urgency without reducing the time available to read the result. |
| `timing-cubic-in` | Cubic In | Build momentum or send an element decisively out of frame. |
| `timing-cubic-out` | Cubic Out | Bring attention into a composition with a readable landing. |
| `timing-cubic-in-out` | Cubic In / Out | Connect two stable states with a smooth, self-contained move. |
| `timing-speed-ramp` | Speed Ramp | Pass quickly through the middle while keeping both ends legible. |
| `timing-overshoot` | Overshoot | Suggest momentum continuing beyond an intended stopping point. |
| `timing-undershoot` | Undershoot | Suggest a cautious arrival or a mechanism finding its final position. |
| `timing-spring` | Spring | Suggest stored energy resolving through several corrections. |
| `timing-stepped` | Stepped | Give motion a deliberate stop-motion or discrete-update character. |
| `timing-glitchy` | Glitchy | Make continuity feel interrupted while keeping the outcome repeatable. |
