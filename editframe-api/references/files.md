...
console.log(file.expires_at); // null = permanent
```

Optional `expires_at` (ISO 8601) schedules automatic removal. Omit for permanent retention. Maximum **30 days** in the future.
...
| `expires_at` | Behavior |
|--------------|----------|
| `null` or omitted | Retained until `deleteFile` |
| Future ISO datetime | Reaped shortly after expiry; content routes return 410 `file_expired` |

`getFileDetail` still returns metadata after expiry. Render-ingested remote URLs get a **one-hour** system TTL (not configurable via upload API).

**Validation:** `createFile` validates `expires_at` server-side and returns `400` on `POST /api/v1/files` for:
...
This validation is identical to `createRender`'s (see `references/renders.md`) — same error codes, same 30-day maximum, same 400 status.

URL signing JWT lifetime is separate from file retention — see `references/url-signing.md`.
...
console.log(detail.expires_at); // null or ISO datetime
