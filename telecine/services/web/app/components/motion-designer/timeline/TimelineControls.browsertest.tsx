...
describe("TimelineControls", () => {
  ...
  describe("Play/Pause Button Rendering", () => {
    test("renders play/pause buttons when previewTargetId is provided", () => {
      ...
      const { container } = render(
        <TimelineControls
          previewTargetId="test-timegroup-1"
          onRestart={onRestart}
        />,
      );
    ...
    test("does not render play/pause buttons when previewTargetId is undefined", () => {
      const onRestart = vi.fn();
      const { container } = render(<TimelineControls onRestart={onRestart} />);
  ...
  describe("Play/Pause Functionality", () => {
    test("clicking play button triggers playback", async () => {
      ...
      const { container } = render(
        <TimelineControls
          previewTargetId="test-timegroup-2"
          onRestart={onRestart}
        />,
      );
      ...
      await waitFor(
        () => {
          ...
          const isPlaying = (timegroupElement as any).playbackController
            ?.playing;
    ...
    test("pause button is clickable when element is playing", async () => {
      ...
      const { container } = render(
        <TimelineControls
          previewTargetId="test-timegroup-3"
          onRestart={onRestart}
        />,
      );
  ...
  describe("Loop Toggle Button Rendering", () => {
    test("renders loop toggle button when previewTargetId is provided", () => {
      ...
      const { container } = render(
        <TimelineControls
          previewTargetId="test-timegroup-4"
          onRestart={onRestart}
        />,
      );
  ...
  describe("Loop Toggle Functionality", () => {
    test("clicking loop button toggles loop state only", async () => {
      const { timegroupElement, playbackController } =
        createMockTimegroupElement("test-timegroup-5");
      ...
      const { container } = render(
        <TimelineControls
          previewTargetId="test-timegroup-5"
          onRestart={onRestart}
        />,
      );
    ...
    test("clicking loop button does not affect playback when playing", async () => {
      const { timegroupElement, playbackController } =
        createMockTimegroupElement("test-timegroup-8", true);

      const onRestart = vi.fn();
      const { container } = render(
        <TimelineControls
          previewTargetId="test-timegroup-8"
          onRestart={onRestart}
        />,
      );
    ...
    test("loop button shows visual feedback when loop is enabled", async () => {
      ...
      const { container } = render(
        <TimelineControls
          previewTargetId="test-timegroup-6"
          onRestart={onRestart}
        />,
      );
      ...
      expect(
        loopButton.offsetParent !== null || loopButton.style.display !== "none",
      ).toBe(true);
    });

    test("loop button shows inactive state when loop is disabled", async () => {
      const { timegroupElement } =
        createMockTimegroupElement("test-timegroup-7");
      ...
      const { container } = render(
        <TimelineControls
          previewTargetId="test-timegroup-7"
          onRestart={onRestart}
        />,
      );
