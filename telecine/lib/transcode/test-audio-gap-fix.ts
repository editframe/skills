...
async function testAudioGapFix() {
  ...
  try {
    ...
    for (let i = 0; i < 3; i++) {
      ...
      await writeFile(
        `output/sine-segment-${i}.mp4`,
        new Uint8Array(result.outputData),
      );
      console.log(
        `💾 Saved segment ${i}: ${result.outputData.byteLength} bytes`,
      );

      // Log timing information
      console.log(`🎯 Segment ${i} Timing:`);
      console.log(
        `   Requested: ${result.alignedTiming.requestedFromUs / 1000}ms - ${result.alignedTiming.requestedToUs / 1000}ms`,
      );
      console.log(
        `   Aligned: ${result.alignedTiming.alignedFromUs / 1000}ms - ${result.alignedTiming.alignedToUs / 1000}ms`,
      );
      console.log(
        `   Duration: ${(result.alignedTiming.alignedToUs - result.alignedTiming.alignedFromUs) / 1000}ms`,
      );
    }
