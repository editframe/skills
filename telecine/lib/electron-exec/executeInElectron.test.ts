...
import { executeInElectron, executeInElectronWithRpc } from "./executeInElectron.ts";
...
describe("executeInElectron", () => {
  ...
  test("executes simple scripts", testSpan(async () => {
  ...
  test("errors propagate up", testSpan(async () => {
    const iterations = 3;
    ...
    for (let i = 0; i < iterations; i++) {
      await executeSpan(`execution-${i}`, async () => {
        ...
        const execution = await executeInElectron(testScript);
        await expect(execution.processExit).rejects.toThrow("Electron process exited with code 1");
      });
    }
  }));

  test("an RPC socket is available", testSpan(async () => {
  ...
  test("RPC keepalive extends timeout for long-running tasks", testSpan(async () => {
    const testScript = "/app/lib/electron-exec/test-script/rpc.ts";
    const execution = await executeInElectronWithRpc(testScript);
