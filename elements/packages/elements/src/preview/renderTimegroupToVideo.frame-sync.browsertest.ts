...
describe("Frame Synchronization in Video Rendering", () => {
  ...
  it(
    ...
    async () => {
      ...
      try {
        ...
        console.log(
          `  - Frame at 400ms:  "ONE TWO THREE" visible (stagger 400ms)`,
        );
        console.log(
          `  - Frame at 600ms:  "ONE TWO THREE FOUR" visible (stagger 600ms)`,
        );
        console.log(
          `  - Frame at 800ms:  "ONE TWO THREE FOUR FIVE" visible (stagger 800ms)`,
        );
        console.log(
          `\n[Frame Sync Test] With the BUG: All frames will show the same state (likely all words visible or none)`,
        );
        console.log(
          `[Frame Sync Test] With the FIX: Each frame will show the correct words for that time`,
        );
  ...
  it(
    "should demonstrate the bug with manual frame capture",
    { timeout: 30000 },
    async () => {
      ...
      tg.style.cssText =
        "width: 400px; height: 300px; background: black; display: block;";
      ...
      try {
        ...
        try {
          ...
          console.log(
            `  After seek to 0ms completed, renderClone is at: ${timeAfterSeek0}ms`,
          );
