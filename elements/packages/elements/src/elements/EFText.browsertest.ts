...
describe("EFText", () => {
  ...
  describe("stagger functionality", () => {
    ...
    test("stagger actually delays animations in character mode (DOM/CSSOM test)", async () => {
      createTestStyle(`
        @keyframes fadeIn {
          from {
            opacity: 0;
      ...
      // CRITICAL: Add animation class to segments
      // This must happen after segments are connected to DOM but before we check for animations
      segments.forEach((segment) => {
        segment.classList.add("fade-in");
      });

      // Wait for segments to be fully rendered
      await Promise.all(segments.map((seg) => seg.updateComplete));
      
      // CRITICAL: Wait for CSS animations to be created
      // CSS animations are created asynchronously when a class is added
      // We need multiple frames to ensure all animations are created
      await new Promise((resolve) => requestAnimationFrame(resolve));
      await new Promise((resolve) => requestAnimationFrame(resolve));
      await new Promise((resolve) => requestAnimationFrame(resolve));
      
      // CRITICAL: Verify animations exist on ALL segments before proceeding
      // This is the core feature - every segment must have animations
      // Check both directly on segments and via timegroup.getAnimations({ subtree: true })
      const timegroupAnimations = timegroup.getAnimations({ subtree: true });
      const segmentAnimationsFromTimegroup = timegroupAnimations.filter((anim) => {
        const target = anim.effect?.target;
        return target && target instanceof HTMLElement && target.tagName === "EF-TEXT-SEGMENT";
      });
      
      // All segments should have animations discoverable via timegroup
      expect(segmentAnimationsFromTimegroup.length).toBe(
        3,
        `Timegroup should find animations on all 3 segments, but found ${segmentAnimationsFromTimegroup.length}`,
      );
      
      // Also verify each segment directly reports having animations
      for (let i = 0; i < segments.length; i++) {
        const segment = segments[i];
        const anims = segment?.getAnimations() ?? [];
        expect(anims.length).toBeGreaterThan(
          0,
          `Segment ${i} (${segment?.segmentText}) must have at least one animation. Found ${anims.length} animations.`,
        );
      }

      // Verify segments have stagger offsets set
      // With stagger="100ms" and 3 characters:
      // Total stagger = (3-1) * 100ms = 200ms
      // First: 0 / 2 * 200ms = 0ms
      // Second: 1 / 2 * 200ms = 100ms
      // Third: 2 / 2 * 200ms = 200ms
      expect(segments[0]?.staggerOffsetMs).toBe(0);
      expect(segments[1]?.staggerOffsetMs).toBe(100);
      expect(segments[2]?.staggerOffsetMs).toBe(200);

      // Get animations for each segment
      const anim0 = segments[0]?.getAnimations()[0];
      const anim1 = segments[1]?.getAnimations()[0];
      const anim2 = segments[2]?.getAnimations()[0];

      expect(anim0).toBeTruthy();
      expect(anim1).toBeTruthy();
      expect(anim2).toBeTruthy();

      // Verify animation targets are the segments themselves
      expect(anim0?.effect?.target).toBe(segments[0]);
      expect(anim1?.effect?.target).toBe(segments[1]);
      expect(anim2?.effect?.target).toBe(segments[2]);

      // Verify target elements have correct tagName and staggerOffsetMs
      const target0 = anim0?.effect?.target as HTMLElement;
      const target1 = anim1?.effect?.target as HTMLElement;
      const target2 = anim2?.effect?.target as HTMLElement;
      expect(target0?.tagName).toBe("EF-TEXT-SEGMENT");
      expect(target1?.tagName).toBe("EF-TEXT-SEGMENT");
      expect(target2?.tagName).toBe("EF-TEXT-SEGMENT");
      expect((target0 as any).staggerOffsetMs).toBe(0);
      expect((target1 as any).staggerOffsetMs).toBe(100);
      expect((target2 as any).staggerOffsetMs).toBe(200);

      // Verify animations have correct timing (delay should include stagger)
      const timing0 = anim0?.effect?.getTiming();
      const timing1 = anim1?.effect?.getTiming();
      const timing2 = anim2?.effect?.getTiming();
      // CSS animations have delay in the timing object
      // The effective delay (including stagger) is applied in updateAnimations
      // So we can't directly check it here, but we can verify the base delay
      expect(timing0?.delay).toBe(0);
      expect(timing1?.delay).toBe(0);
      expect(timing2?.delay).toBe(0);

      // At t=0, all animations should be at initial state (opacity 0, translateY 20px)
      await timegroup.seek(0);
      await Promise.all(
        segments.map((seg) => seg.frameTask?.taskComplete).filter((p) => p),
      );

      // Check computed styles - all should be at initial state
      const style0Initial = window.getComputedStyle(segments[0]!);
      const style1Initial = window.getComputedStyle(segments[1]!);
      const style2Initial = window.getComputedStyle(segments[2]!);

      // All should have opacity near 0 and transform translateY(20px) at t=0
      expect(parseFloat(style0Initial.opacity)).toBeLessThan(0.1);
      expect(parseFloat(style1Initial.opacity)).toBeLessThan(0.1);
      expect(parseFloat(style2Initial.opacity)).toBeLessThan(0.1);

      // Verify timegroup time is set correctly
      await timegroup.seek(250);
      // Wait for all frame tasks to complete to ensure animations are synchronized
      await Promise.all(
        segments.map((seg) => seg.frameTask?.taskComplete).filter((p) => p),
      );
      // Also wait for any pending animation updates
      await new Promise((resolve) => requestAnimationFrame(resolve));
      await new Promise((resolve) => requestAnimationFrame(resolve));

      // Get actual timegroup time (may vary slightly)
      const timegroupTime = timegroup.ownCurrentTimeMs;
      expect(timegroupTime).toBeGreaterThan(240);

      // CRITICAL: Verify all segments have animations
      // This is the root cause - if segments don't have animations, stagger can't work
      const allAnimations = timegroup.getAnimations({ subtree: true });
      const segmentAnimations = allAnimations.filter((anim) => {
        const target = anim.effect?.target;
        return target && target instanceof HTMLElement && target.tagName === "EF-TEXT-SEGMENT";
      });
      
      // All three segments should have animations
      expect(segmentAnimations.length).toBe(3);
      
      // Verify each segment has at least one animation
      const segment0Anims = segments[0]?.getAnimations() ?? [];
      const segment1Anims = segments[1]?.getAnimations() ?? [];
      const segment2Anims = segments[2]?.getAnimations() ?? [];
      
      expect(segment0Anims.length).toBeGreaterThan(0);
      expect(segment1Anims.length).toBeGreaterThan(0);
      expect(segment2Anims.length).toBeGreaterThan(0);
      
      // Verify segments are rendered and CSS variables are set correctly
      // Wait for segments to render CSS variables
      await Promise.all(segments.map((seg) => seg.updateComplete));
      await new Promise((resolve) => requestAnimationFrame(resolve));
      
      // Verify CSS variables are set correctly on all segments
      // Read from the element's style directly, not computed style
      // The CSS variable is set via style.setProperty, so it should be in the inline style
      const cssVar0 = segments[0]?.style.getPropertyValue("--ef-stagger-offset").trim();
      const cssVar1 = segments[1]?.style.getPropertyValue("--ef-stagger-offset").trim();
      const cssVar2 = segments[2]?.style.getPropertyValue("--ef-stagger-offset").trim();
      
      // Also check computed style as fallback
      const computedVar0 = window.getComputedStyle(segments[0]!).getPropertyValue("--ef-stagger-offset").trim();
      const computedVar1 = window.getComputedStyle(segments[1]!).getPropertyValue("--ef-stagger-offset").trim();
      const computedVar2 = window.getComputedStyle(segments[2]!).getPropertyValue("--ef-stagger-offset").trim();
      
      // Use inline style if available, otherwise computed style
      const finalVar0 = cssVar0 || computedVar0;
      const finalVar1 = cssVar1 || computedVar1;
      const finalVar2 = cssVar2 || computedVar2;
      
      expect(finalVar0).toBe("0ms");
      expect(finalVar1).toBe("100ms");
      expect(finalVar2).toBe("200ms");
      
      // Verify animations have been synchronized with stagger
      // Check animation currentTime to verify stagger is applied
      const anim0Time = anim0?.currentTime ?? 0;
      const anim1Time = anim1?.currentTime ?? 0;
      const anim2Time = anim2?.currentTime ?? 0;
      
      // All animations should have currentTime set (not null)
      expect(anim0Time).toBeGreaterThan(0);
      expect(anim1Time).toBeGreaterThan(0);
      expect(anim2Time).toBeGreaterThan(0);
      
      // With stagger, animations should have different currentTime values
      // First: no stagger, so currentTime should be ~250ms (timegroupTime)
      // Second: 100ms stagger, so currentTime should be ~250ms (100ms delay + 150ms animation time)
      // Third: 200ms stagger, so currentTime should be ~250ms (200ms delay + 50ms animation time)
      // But the animation progress (how far into the 500ms animation) differs:
      // First: 250ms into animation
      // Second: 150ms into animation (250 - 100 stagger)
      // Third: 50ms into animation (250 - 200 stagger)
      
      // All should have currentTime around 250ms (absolute timeline time)
      expect(anim0Time).toBeGreaterThan(240);
      expect(anim1Time).toBeGreaterThan(240);
      expect(anim2Time).toBeGreaterThan(240);

      // Verify computed styles reflect the stagger - first should be more progressed
      const style0Stagger = window.getComputedStyle(segments[0]!);
      const style1Stagger = window.getComputedStyle(segments[1]!);
      const style2Stagger = window.getComputedStyle(segments[2]!);

      // At this timeline time, stagger should cause different animation progress:
      // First: no stagger, so ~timegroupTime ms into animation
      // Second: 100ms stagger, so ~(timegroupTime - 100) ms into animation  
      // Third: 200ms stagger, so ~(timegroupTime - 200) ms into animation
      // First should be further along than second, second further than third
      const opacity0 = parseFloat(style0Stagger.opacity);
      const opacity1 = parseFloat(style1Stagger.opacity);
      const opacity2 = parseFloat(style2Stagger.opacity);

      // Verify stagger is working: first should be most progressed, third least
      expect(opacity0).toBeGreaterThan(opacity1);
      expect(opacity1).toBeGreaterThan(opacity2);
      
      // All should have started animating (opacity > 0) since timegroupTime > 200
      expect(opacity0).toBeGreaterThan(0);
      expect(opacity1).toBeGreaterThan(0);
      expect(opacity2).toBeGreaterThan(0);
    });

    test("template with animation classes: each character animates with stagger", async () => {
      // This test matches the real-world scenario where segments are cloned from a template
      // with animation classes, and verifies that ALL characters animate with stagger
      createTestStyle(`
        @keyframes bounceIn {
          0% {
      ...
      timegroup.duration = "6s";
      const text = document.createElement("ef-text");
      text.split = "char";
      text.setAttribute("stagger", "50ms");
      text.duration = "6s";
      
      // Create template with animation class (matches user's example)
      const template = document.createElement("template");
      const segment = document.createElement("ef-text-segment");
      segment.className = "typewriter";
      template.appendChild(segment);
      text.appendChild(template);
      
      // Use only 2 characters for simpler testing
      text.textContent = "AB";
      ...
      // CRITICAL: Wait for CSS animations to be created on cloned segments
      // This is the core feature - every segment cloned from template must have animations
      // CSS animations are created asynchronously, so we need to wait and verify
      let allSegmentsHaveAnimations = false;
      for (let attempt = 0; attempt < 20; attempt++) {
        await new Promise((resolve) => requestAnimationFrame(resolve));
        
        // Check if all segments with classes have animations
        const segmentsWithClasses = segments.filter((seg) => seg.classList.length > 0);
        const segmentsWithAnimations = segmentsWithClasses.filter((seg) => {
          const anims = seg.getAnimations();
          return anims.length > 0;
        });
        
        if (segmentsWithAnimations.length === segmentsWithClasses.length && segmentsWithClasses.length > 0) {
          allSegmentsHaveAnimations = true;
          break;
        }
      }

      // CRITICAL: Verify ALL segments have animations (not just the first one)
      // This is the regression we're fixing - all segments must animate
      const timegroupAnimations = timegroup.getAnimations({ subtree: true });
      const segmentAnimationsFromTimegroup = timegroupAnimations.filter((anim) => {
        const target = anim.effect?.target;
        return target && target instanceof HTMLElement && target.tagName === "EF-TEXT-SEGMENT";
      ...
      // Both segments should have animations discoverable via timegroup
      expect(segmentAnimationsFromTimegroup.length).toBe(
        2,
        `Timegroup should find animations on all 2 segments, but found ${segmentAnimationsFromTimegroup.length}. All segments have animations: ${allSegmentsHaveAnimations}`,
      );
      
      // Verify each segment directly reports having animations
      // CRITICAL: This is the core feature - every segment must have animations
      for (let i = 0; i < segments.length; i++) {
        const segment = segments[i];
        const anims = segment?.getAnimations() ?? [];
        const hasClass = segment.classList.length > 0;
        expect(anims.length).toBeGreaterThan(
          0,
          `Segment ${i} (${segment?.segmentText}) must have at least one animation. Found ${anims.length} animations. Has class: ${hasClass}, Classes: ${segment.className}. All segments have animations: ${allSegmentsHaveAnimations}`,
        );
      }

      // Verify stagger offsets are set correctly
      // With stagger="50ms" and 2 characters:
      // Total stagger = (2-1) * 50ms = 50ms
      // First: 0 / 1 * 50ms = 0ms
      // Second: 1 / 1 * 50ms = 50ms
      expect(segments[0]?.staggerOffsetMs).toBe(0);
      expect(segments[1]?.staggerOffsetMs).toBe(50);
      ...
      // At t=0, all animations should be at initial state (opacity 0)
      await timegroup.seek(0);
      await Promise.all(
        segments.map((seg) => seg.frameTask?.taskComplete).filter((p) => p),
      );
      ...
      // All should have opacity near 0 at t=0
      expect(parseFloat(style0Initial.opacity)).toBeLessThan(0.1);
      expect(parseFloat(style1Initial.opacity)).toBeLessThan(0.1);

      // Seek to a time where stagger should be visible
      // At 75ms: first segment should be animating (75ms > 0ms), second should be starting (75ms > 50ms)
      await timegroup.seek(75);
      await Promise.all(
        segments.map((seg) => seg.frameTask?.taskComplete).filter((p) => p),
      );
      await new Promise((resolve) => requestAnimationFrame(resolve));
      ...
      // Verify computed styles reflect the stagger
      const style0Stagger = window.getComputedStyle(segments[0]!);
      const style1Stagger = window.getComputedStyle(segments[1]!);
      ...
      // First segment should be more progressed than second (started 50ms earlier)
      expect(opacity0).toBeGreaterThan(opacity1);
      
      // Both should have started animating (opacity > 0) since timegroupTime > 50ms
      expect(opacity0).toBeGreaterThan(0);
      expect(opacity1).toBeGreaterThan(0);

      // Verify stagger is actually delaying the second segment
      // At 25ms: first should be animating, second should still be at initial state
      await timegroup.seek(25);
      await Promise.all(
        segments.map((seg) => seg.frameTask?.taskComplete).filter((p) => p),
      );
      await new Promise((resolve) => requestAnimationFrame(resolve));
      ...
      const style0Early = window.getComputedStyle(segments[0]!);
      const style1Early = window.getComputedStyle(segments[1]!);

      const opacity0Early = parseFloat(style0Early.opacity);
      const opacity1Early = parseFloat(style1Early.opacity);

      // First should be animating (started at 0ms), second should still be at initial state (starts at 50ms)
      expect(opacity0Early).toBeGreaterThan(0);
      expect(opacity1Early).toBeLessThan(0.1);
    });
  });
