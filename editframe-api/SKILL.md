...
Optional `expires_at` on `createFile` (ISO 8601, max 30 days ahead) schedules automatic removal; omit for permanent retention. File webhooks and `getFileDetail` include `expires_at` (`null` when permanent).

Renders support the same optional `expires_at` retention field; `deleteRender` removes a render and all of its GCS output immediately.
