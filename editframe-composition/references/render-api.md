...
- **getRenderData<T>()** - Access custom data passed at render time (CLI --data, bundleRender renderData, or window.EF_RENDER_DATA). Use useRenderData() in React components instead.
  - Returns: T | undefined

## Functions

- **useRenderData<T>()** - React hook equivalent of getRenderData(). Returns data injected at render time. Stable reference — reads once on mount. Import from @editframe/react.
  - Returns: T | undefined
