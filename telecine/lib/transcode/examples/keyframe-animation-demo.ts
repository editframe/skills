...
class KeyframeAnimationSystem {
  ...
  addTrack(property: 'translateX' | 'translateY' | 'rotate', unit: string = 'px'): AnimationTrack {
  ...
  moveKeyframe(track: AnimationTrack, oldTimeMs: number, newTimeMs: number): boolean {
    const keyframe = track.keyframes.find(kf => kf.timeMs === oldTimeMs);
    if (!keyframe) return false;

    keyframe.timeMs = Math.max(0, Math.min(newTimeMs, this.timeGroup.durationMs));
    track.keyframes.sort((a, b) => a.timeMs - b.timeMs);
    return true;
  }
  ...
  getTimelineView(): string {
    ...
    for (const track of this.timeGroup.tracks) {
      ...
      for (const kf of track.keyframes) {
        const position = Math.round((kf.timeMs / duration) * (timelineWidth - 1));
        trackLine[position] = '●';
      }

      const trackDisplay = trackLine.join('');
      lines.push(`${track.property}${' '.repeat(labelPadding)}│${trackDisplay}│`);
  ...
  toCSSAnimation(animationName: string): string {
    ...
    css.push(`  animation: ${animationName} ${duration}ms ease-in-out infinite;`);
    css.push(`}`);
