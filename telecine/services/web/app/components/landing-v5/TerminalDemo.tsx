...
const ANIMATION_STEPS: AnimationStep[] = [
  ...
  { type: "pause", duration: 400 },
  {
    ...
    lines: [
      {
        text: "? Project name: my-video-app",
        className: "text-slate-500",
        delay: 0,
      },
      {
        text: "? Template: Social Clip",
        className: "text-slate-500",
        delay: 300,
      },
    ],
  ...
  { type: "pause", duration: 600 },
  {
    ...
    lines: [
      ...
      {
        text: "Preview: http://localhost:3000/preview",
        className: "text-slate-500",
        delay: 200,
      },
    ],
...
function TerminalDemo() {
  ...
  const [visibleOutputs, setVisibleOutputs] = useState<
    { stepIndex: number; lineIndex: number }[]
  >([]);
  ...
  useEffect(() => {
    ...
    if (step.type === "type") {
      ...
      const stepOutputs = visibleOutputs.filter(
        (o) => o.stepIndex === currentStepIndex,
      );
      ...
      if (nextLineIndex < step.lines.length) {
        ...
        const delay =
          nextLineIndex === 0 ? (step.delay ?? 0) : (line?.delay ?? 100);

        timeoutRef.current = setTimeout(() => {
          setVisibleOutputs((prev) => [
            ...prev,
            { stepIndex: currentStepIndex, lineIndex: nextLineIndex },
          ]);
        }, delay);
      } else {
        setCurrentStepIndex((prev) => prev + 1);
                    ...
                    <p key={lineIndex} className="text-white/70">
                      {parts.map((part, i) =>
                        part.startsWith("http://") ? (
                          <span key={i} className="text-[var(--accent-blue)]">
                            {part}
                          </span>
                        ) : (
                          part
