...
describe('AudioTiming', () => {
  ...
  describe('getClosestAlignedTimeUs', () => {
    test('aligns to exact AAC frame boundaries', () => {
      ...
      expect(getClosestAlignedTimeUs(AUDIO_FRAME_DURATION_US)).toBeCloseTo(AUDIO_FRAME_DURATION_US, 6);
      expect(getClosestAlignedTimeUs(AUDIO_FRAME_DURATION_US * 2)).toBeCloseTo(AUDIO_FRAME_DURATION_US * 2, 6);
    });
    ...
    test('rounds to nearest frame boundary', () => {
      ...
      expect(getClosestAlignedTimeUs(halfFrame * 1.2)).toBeCloseTo(AUDIO_FRAME_DURATION_US, 6);
    });
  ...
  describe('calculateAlignedTiming', () => {
    test('calculates timing for init segment without padding', () => {
      ...
      expect(result.alignedToUs).toBeCloseTo(getClosestAlignedTimeUs(1000 * 1000), 6);
    });
  ...
  describe('calculateAudioTiming', () => {
    test('calculates timing for first segment without start trimming', () => {
      ...
      expect(result.outpointUs).toBeCloseTo(alignedDurationUs - AUDIO_FRAME_DURATION_US, 6);
      expect(result.durationUs).toBeCloseTo(result.outpointUs - result.inpointUs, 6);
    });
    ...
    test('calculates timing for subsequent segments with start trimming', () => {
      ...
      const expectedDuration = Math.max(0, result.outpointUs - result.inpointUs);
  ...
  describe('Integration Tests', () => {
    test('end-to-end timing calculation matches SegmentEncoder logic', () => {
      ...
      const alignedDurationUs = alignedTiming.alignedToUs - alignedTiming.alignedFromUs;
      const audioTiming = calculateAudioTiming(alignedDurationUs, segmentOptions.sequenceNumber);
    ...
    test('refactored SegmentEncoder timing calculations are identical to original', () => {
      ...
      const testCases = [
        { segmentStartMs: 0, segmentEndMs: 2000, sequenceNumber: 0, isInitSegment: false },
        { segmentStartMs: 2000, segmentEndMs: 4000, sequenceNumber: 1, isInitSegment: false },
        { segmentStartMs: 4000, segmentEndMs: 6000, sequenceNumber: 2, isInitSegment: false },
        { segmentStartMs: 0, segmentEndMs: 100, sequenceNumber: 0, isInitSegment: true },
      ];

      testCases.forEach(({ segmentStartMs, segmentEndMs, sequenceNumber, isInitSegment }) => {
        ...
        const alignedDurationUs = alignedTiming.alignedToUs - alignedTiming.alignedFromUs;
        const audioTiming = calculateAudioTiming(alignedDurationUs, sequenceNumber);

        // Verify timing calculations are consistent with SegmentEncoder expectations
        expect(alignedTiming.paddedStart).toBe(!isInitSegment && sequenceNumber > 0);
        ...
        if (!isInitSegment) {
          if (sequenceNumber > 0) {
            ...
            const expectedAlignedFromUs = getClosestAlignedTimeUs(segmentStartMs * 1000) - AUDIO_FRAME_DURATION_US * 2;
            expect(alignedTiming.alignedFromUs).toBeCloseTo(expectedAlignedFromUs, 6);
          }

          // Should have 2 frames of end padding
          const expectedAlignedToUs = getClosestAlignedTimeUs(segmentEndMs * 1000) + AUDIO_FRAME_DURATION_US * 2;
          expect(alignedTiming.alignedToUs).toBeCloseTo(expectedAlignedToUs, 6);
        }

        // Verify audio timing for concat directive
        if (sequenceNumber > 0) {
          expect(audioTiming.inpointUs).toBeCloseTo(AUDIO_FRAME_DURATION_US * 4, 6);
        } else {
          expect(audioTiming.inpointUs).toBe(0);
        }

        expect(audioTiming.outpointUs).toBeCloseTo(alignedDurationUs - AUDIO_FRAME_DURATION_US, 6);
