...
Render event payloads include `expires_at` (`null` for permanent renders).
...
File event payloads include `expires_at` (`null` for permanent uploads, or an ISO 8601 datetime). See `references/events.md` and `editframe-api` `references/files.md` for retention rules.
