...
} from "../../../elements/src/preview/previewSettings";
 ...
 */
function getCanvasAverageColor(source: CanvasImageSource): {
  r: number;
  g: number;
  b: number;
} {
...
describe("TimelineRoot", () => {
  describe.runIf(isNativeCanvasApiAvailable())(
    "video export with native rendering",
    () => {
      test("renderTimegroupToVideo with React TimelineRoot + Configuration (matches design-catalog setup)", async () => {
        ...
        try {
          ...
          root.render(
            <TimelineRoot
              id="test-timeline"
              component={ColorChangingContent}
            />,
          );
          ...
          const capture1 = await captureTimegroupAtTime(timegroup, {
            timeMs: 100,
            scale: 0.5,
          });
          const capture2 = await captureTimegroupAtTime(timegroup, {
            timeMs: 600,
            scale: 0.5,
          });
      ...
      test("captures different frames at different timestamps (design catalog like)", async () => {
        ...
        try {
          ...
          root.render(
            <TimelineRoot
              id="test-catalog"
              component={DesignCatalogLikeContent}
            />,
          );
          ...
          console.log(
            "[Test Catalog] Timegroup duration:",
            timegroup.durationMs,
          );
          console.log(
            "[Test Catalog] Nested timegroups:",
            timegroup.querySelectorAll("ef-timegroup").length,
          );

          // Capture at three different times: 100ms (red), 600ms (green), 1100ms (blue)
          console.log(
            "[Test Catalog] Starting captures at 100ms, 600ms, 1100ms",
          );
          const capture1 = await captureTimegroupAtTime(timegroup, {
            timeMs: 100,
            scale: 0.5,
          ...
          const capture3 = await captureTimegroupAtTime(timegroup, {
            timeMs: 1100,
            scale: 0.5,
          });

          // Get colors from all captures
          const colors = [capture1, capture2, capture3].map((c) =>
            getCanvasAverageColor(c),
          );
          ...
          const diff12 =
            Math.abs(colors[0]!.r - colors[1]!.r) +
            Math.abs(colors[0]!.g - colors[1]!.g) +
            Math.abs(colors[0]!.b - colors[1]!.b);
          const diff23 =
            Math.abs(colors[1]!.r - colors[2]!.r) +
            Math.abs(colors[1]!.g - colors[2]!.g) +
            Math.abs(colors[1]!.b - colors[2]!.b);
          ...
          assert.isAbove(
            diff12,
            30,
            `Frames 1 and 2 should have different colors`,
          );
          assert.isAbove(
            diff23,
            30,
      ...
      test("clone is created with React content that responds to seek", async () => {
        ...
        try {
          ...
          await clone.seek(0);
          console.log(
            "[Test] After seek(0), clone.currentTimeMs:",
            clone.currentTimeMs,
          );
          ...
          console.log(
            "[Test] After seek(600), clone.currentTimeMs:",
            clone.currentTimeMs,
          );
      ...
      test.runIf(isNativeCanvasApiAvailable())(
        "captures different frames using NATIVE path (verified)",
        async () => {
          // Ensure native API is enabled
          setNativeCanvasApiEnabled(true);
          ...
          try {
            ...
            assert.isTrue(
              isNativeCanvasApiAvailable(),
              "Native canvas API should be available",
            );

            console.log(
              "[Test Native] Timegroup duration:",
              timegroup.durationMs,
            );
            console.log(
              "[Test Native] Native API available:",
              isNativeCanvasApiAvailable(),
            );
            ...
            console.log(
              "[Test Native] Canvas 1 (100ms) average color:",
              color1,
            );
            console.log(
              "[Test Native] Canvas 2 (600ms) average color:",
              color2,
            );
      ...
      test("captures different frames using FOREIGNOBJECT path (forced)", async () => {
        ...
        try {
          ...
          console.log(
            "[Test ForeignObject] Timegroup duration:",
            timegroup.durationMs,
          );
          console.log(
            "[Test ForeignObject] Using foreignObject path (native disabled)",
          );

          // Capture at two different times: 100ms (should be red) and 600ms (should be blue)
          console.log(
            "[Test ForeignObject] Starting captures at 100ms and 600ms",
          );
          const capture1 = await captureTimegroupAtTime(timegroup, {
            timeMs: 100,
            scale: 0.5,
          ...
          console.log(
            "[Test ForeignObject] Canvas 1 (100ms) average color:",
            color1,
          );
          console.log(
            "[Test ForeignObject] Canvas 2 (600ms) average color:",
            color2,
          );
          ...
          const colorDifference =
            ...
            Math.abs(color1.b - color2.b);
          console.log(
            "[Test ForeignObject] Color difference:",
            colorDifference,
          );
    ...
    () => {
      test("renderTimegroupToVideo with React TimelineRoot + Configuration (matches design-catalog setup)", async () => {
        ...
        try {
          ...
          console.log(
            "[Video Export Test] Timegroup duration:",
            timegroup.durationMs,
          );
          console.log(
            "[Video Export Test] Native API available:",
            isNativeCanvasApiAvailable(),
          );
          ...
          assert.isAbove(
            videoBuffer!.length,
            1000,
            "Video buffer should have content",
          );

          console.log(
            "[Video Export Test] Video export succeeded, buffer size:",
            videoBuffer!.length,
          );
        } catch (error: any) {
          // Check if it's the specific layout error we're trying to reproduce
          if (
            error.message?.includes("drawElementImage") &&
            error.message?.includes("laid out")
          ) {
            console.error(
              "[Video Export Test] REPRODUCED ERROR:",
              error.message,
            );
