...
import { frameDuration, getClosestAlignedTime, generateCommandAndDirectivesForSegment } from './calculations';
...
if (process.argv.length === 3 && !/^-?\d+(\.\d+)?$/.test(process.argv[2])) {
  ...
  targetSegmentDuration = parseFloat(process.argv[2] || DEFAULT_SEGMENT_DURATION.toString());
}
...
const segmentTimings: Array<{ startTime: number; endTime: number; isLast: boolean }> = [];

for (let i = 0; i < numSegments; i++) {
  const startTime = Math.round(i * targetSegmentDuration * 1000000);
  const endTime = Math.min(Math.round((i + 1) * targetSegmentDuration * 1000000), duration * 1000000);
  const isLast = i === numSegments - 1;

  // Store timing information for video extraction later
  segmentTimings.push({ startTime, endTime, isLast });

  const result = generateCommandAndDirectivesForSegment(inputFile!, i, startTime, endTime, isLast);
  commandsAndDirectives.push(result);
}

const allDirectives = commandsAndDirectives.map(([cmd, directives]) => directives).join('\n');
...
console.log('Creating individual MP4 files with audio and video for each segment...');
commandsAndDirectives.forEach(([_, directives], index) => {
  ...
  if (!inpointLine || !outpointLine) {
    throw new Error(`Could not find inpoint/outpoint in directives for segment ${segmentIndex}`);
  }
...
if (fs.existsSync(firstMp4File)) {
  ...
  const initSegmentBytes = repackageInitSegment(firstMp4IsoFile, totalDurationMs);
...
for (let i = 0; i < numSegments; i++) {
  ...
  if (fs.existsSync(mp4File)) {
    ...
    const mediaSegmentBytes = repackageMediaSegment(mp4IsoFile, i, baseMediaDecodeTimeMs);
...
const concatCmd = 'ffmpeg -hide_banner -loglevel error -nostats -y -f concat -i out/audio-concat.txt -c copy out/stitched.mp4';
