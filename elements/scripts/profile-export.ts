...
async function main() {
  ...
  try {
    ...
    // Click export button or trigger export programmatically
    const exportPromise = page.evaluate(async ({ maxDur, benchmark }) => {
      const workbench = document.querySelector("ef-workbench") as any;
      if (workbench?.startExport) {
        // Limit export duration for profiling
        const timegroup = document.querySelector("ef-timegroup") as any;
        const exportDuration = Math.min(timegroup.durationMs, 6000); // Cap at 6s for profiling
        await workbench.startExport({
          toMs: exportDuration,
          streaming: false,
          includeAudio: false, // Skip audio for faster profiling
          benchmarkMode: benchmark,
        });
        return { success: true };
      }
      
      // Fallback: trigger export directly on timegroup
      const timegroup = document.querySelector("ef-timegroup") as any;
    ...
    const result = await Promise.race([
      ...
      new Promise<{ success: false; error: string }>((resolve) =>
        setTimeout(() => resolve({ success: false, error: "Timeout" }), maxDuration)
      ),
    ]);
