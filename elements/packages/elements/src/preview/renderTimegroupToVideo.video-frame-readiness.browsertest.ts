    ...
    try {
      ...
      try {
        ...
        for (const timeMs of testTimes) {
          ...
          if (shadowCanvas) {
            console.log(`[Frame Readiness Test] Canvas dimensions: ${shadowCanvas.width}x${shadowCanvas.height}`);
            
            // Check if canvas has any pixels
            const ctx = shadowCanvas.getContext("2d");
            if (ctx && shadowCanvas.width > 0 && shadowCanvas.height > 0) {
              const imageData = ctx.getImageData(0, 0, Math.min(10, shadowCanvas.width), Math.min(10, shadowCanvas.height));
