...
function calculateBoundsWithModeAwareConstraints(
  context: ResizeContext,
  handle: ResizeHandle,
): BoxBounds {
...
function simpleConstrainBounds(
  bounds: BoxBounds,
  container: Dimensions,
  minSize: number,
): BoxBounds {
...
function deriveDragState(facts: DragFacts) {
  ...
  const movement =
    facts.currentPoint && facts.interaction
      ? {
  ...
  const hasMovedEnoughToBeConsideredDrag =
    movement &&
...
function calculateNewBoundsFromFacts(
  ...
  if (derived.dragMode === "resize" && derived.resizeHandle) {
    const aspectRatio =
      derived.initialBounds.width / derived.initialBounds.height;
    ...
    return calculateBoundsWithModeAwareConstraints(
      context,
      derived.resizeHandle,
    );
  }
...
function useResizeDrag(
  ...
  const globalPointerMove = useCallback(
    (e: PointerEvent) => {
      if (!facts.interaction || e.pointerId !== facts.interaction.pointerId)
        return;
      ...
      const newBounds = calculateNewBoundsFromFacts(
        updatedFacts,
        containerDimensions,
        minSize,
      );
  ...
  const globalPointerUp = useCallback(
    (e: PointerEvent) => {
      if (!facts.interaction || e.pointerId !== facts.interaction.pointerId)
        return;
...
const HANDLE_CONFIGS = [
  {
    type: "top-left" as ResizeHandle,
    className:
      "absolute top-0 left-0 w-3 h-3 bg-blue-400 cursor-nw-resize touch-none -translate-x-1/2 -translate-y-1/2",
  },
  {
    type: "top-right" as ResizeHandle,
    className:
      "absolute top-0 right-0 w-3 h-3 bg-blue-400 cursor-ne-resize touch-none translate-x-1/2 -translate-y-1/2",
  },
  {
    type: "bottom-left" as ResizeHandle,
    className:
      "absolute bottom-0 left-0 w-3 h-3 bg-blue-400 cursor-sw-resize touch-none -translate-x-1/2 translate-y-1/2",
  },
  {
    type: "bottom-right" as ResizeHandle,
    className:
      "absolute bottom-0 right-0 w-3 h-3 bg-blue-400 cursor-se-resize touch-none translate-x-1/2 translate-y-1/2",
  },
  {
    type: "top" as ResizeHandle,
    className:
      "absolute top-0 left-0 w-full h-2 cursor-n-resize touch-none -translate-y-1",
  },
  {
    type: "right" as ResizeHandle,
    className:
      "absolute top-0 right-0 w-2 h-full cursor-e-resize touch-none translate-x-1",
  },
  {
    type: "bottom" as ResizeHandle,
    className:
      "absolute bottom-0 left-0 w-full h-2 cursor-s-resize touch-none translate-y-1",
  },
  {
    type: "left" as ResizeHandle,
    className:
      "absolute top-0 left-0 w-2 h-full cursor-w-resize touch-none -translate-x-1",
  },
] as const;
...
export function ResizableBox({
  ...
  const { isDragging, dragMode, handlePointerDown } = useResizeDrag(
    bounds,
    containerDimensions,
    minSize,
    onBoundsChange,
