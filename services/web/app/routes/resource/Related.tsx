...
interface ResourceDetailWrapperProps {
  ...
  orgId: string;
  id: string;
  relatedType: keyof typeof ResourceModules;
  relId: string;
}
...
export const ResourceDetailWrapper = ({
  ...
  const subscription = useSubscriptionForQuery(
    liveQuery.token,
    ResourceModules[relatedType].detail.query as ProgressiveQueryDescriptor<
      any,
      any
    >,
    { id: relId, orgId },
  ...
  return (
    <div className="mx-auto p-4">
      {ResourceModules[relatedType].detail.fields.map((field) => (
        <InfoRow
          key={field.name}
          label={field.name}
          value={
