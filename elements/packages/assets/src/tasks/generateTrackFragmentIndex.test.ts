...
describe("generateTrackFragmentIndex", () => {
  test("should generate fragment index", async () => {
    const fragmentIndex = await generateTrackFragmentIndexFromPath(
    ...
    for (const trackId of trackIds) {
      ...
      assert.equal(
        ...
        0,
        `Track ${trackId} init should start at 0`,
      );
      assert.isAbove(
        track.initSegment.size,
        0,
        `Track ${trackId} init should have size`,
      );

      // Verify segments
      assert.isArray(
        track.segments,
        `Track ${trackId} should have segments array`,
      );
      assert.isAbove(
        track.segments.length,
        0,
        `Track ${trackId} should have segments`,
      );
      ...
      for (const segment of track.segments) {
        assert.isNumber(
          segment.cts,
          `Track ${trackId} segment should have cts`,
        );
        assert.isNumber(
          segment.dts,
          `Track ${trackId} segment should have dts`,
        );
        assert.isNumber(
          segment.duration,
          `Track ${trackId} segment should have duration`,
        );
        assert.isNumber(
          segment.offset,
          `Track ${trackId} segment should have offset`,
        );
        assert.isNumber(
          segment.size,
          `Track ${trackId} segment should have size`,
        );
      }
      ...
      if (track.type === "video") {
        assert.isNumber(
          track.width,
          `Video track ${trackId} should have width`,
        );
        assert.isNumber(
          track.height,
          `Video track ${trackId} should have height`,
        );
      } else if (track.type === "audio") {
        assert.isNumber(
          track.channel_count,
          `Audio track ${trackId} should have channel_count`,
        );
        assert.isNumber(
          track.sample_rate,
          `Audio track ${trackId} should have sample_rate`,
        );
        assert.isNumber(
          track.sample_size,
          `Audio track ${trackId} should have sample_size`,
        );
      }
    }
  });
