  ...
  try {
    ...
    try {
      ...
      const output = execSync(
        `ffprobe -v quiet -select_streams v:0 -show_entries stream=nb_frames -print_format csv=p=0 "${tempPath}"`,
        { encoding: "utf8" },
      );
    ...
    try {
      ...
      const output = execSync(
        `ffprobe -v quiet -select_streams v:0 -show_entries packet=pts -print_format csv=p=0 "${tempPath}"`,
        { encoding: "utf8" },
      );
      const lines = output
        .trim()
    ...
    try {
      ...
      if (videoStream) {
        ...
        const framerate =
          Number.parseFloat(framerateParts[0] || "30") /
          Number.parseFloat(framerateParts[1] || "1");
        unlinkSync(tempPath);
 ...
 */
export const analyzeBarsPattern = (
  frameBuffer: Buffer,
): BarsPatternAnalysis => {
  ...
  const brightness =
    frameBuffer.subarray(0, 1000).reduce((sum, byte) => sum + byte, 0) /
    1000 /
    255;
 ...
 */
export const testVideoPlayback = async (
  videoPath: string,
 ...
 */
export const extractCodecInfo = async (
  videoPath: string,
): Promise<CodecInfo> => {
  try {
    const output = execSync(
