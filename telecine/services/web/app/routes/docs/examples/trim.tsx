  ...
  const handleTrimDrag = (e: React.PointerEvent, type: "start" | "end") => {
    ...
    const handlePointerMove = (moveEvent: PointerEvent) => {
      ...
      if (type === "start") {
        ...
        newTime = Math.min(
          videoDuration,
          Math.max(newTime, state.startTime + 0.1),
        );
          ...
          <div className="mx-2 mb-2">
            {!state.trimMode ? (
              ...
              <div className="flex items-center gap-4 bg-white rounded-lg border border-gray-200 shadow-sm overflow-hidden px-2 py-2">
                ...
                <button
                  onClick={() =>
                    setState((prev) => ({ ...prev, trimMode: true }))
                  }
              ...
              <div className="bg-white rounded-lg border border-gray-200 shadow-sm p-3">
                ...
                <div className="relative mb-3">
                  ...
                  <div className="absolute inset-0">
                    ...
                    <div
                      ...
                      style={{
                        left: `calc(${(state.endTime / getVideoDuration()) * 100}% - 0.75rem)`,
                      }}
                ...
                <div className="flex justify-between items-center text-xs">
                  <div>
                    <div>
                      In: {formatTime(state.startTime)} • Out:{" "}
                      {formatTime(state.endTime)} • Duration:{" "}
                      {formatTime(state.endTime - state.startTime)}
                    </div>
                    <div className="text-gray-500 text-xs mt-1">
                      Drag green/red handles to trim • Drag yellow area to move
                      • View thumbnails while trimming
                    </div>
                  </div>
                  <button
                    onClick={() =>
                      setState((prev) => ({ ...prev, trimMode: false }))
                    }
