...
import { fetchMoovAndFtyp, fetchLocalMoovAndFtyp, fetchMoovAndFtypUnified, buildFakeMp4, buildMp4FromSegment, createDummyMdat } from './moovScanner.js';
...
describe('MOOV Scanner', () => {
  ...
  const getLocalPath = (filename: string) => `/app/test-assets/transcode/${filename}`;
  ...
  describe('Error Handling', () => {
    ...
    describe('File system errors', () => {
      test('handles non-existent files', async () => {
        const nonExistentPath = '/app/test-assets/transcode/non-existent-file.mp4';
      ...
      test('handles directories instead of files', async () => {
        ...
        await expect(fetchLocalMoovAndFtyp(directoryPath)).rejects.toThrow('Path is not a file');
      });
    ...
    describe('Unified scanner error handling', () => {
      ...
      test('propagates file system errors through unified interface', async () => {
        ...
        await expect(fetchMoovAndFtypUnified(nonExistentPath)).rejects.toThrow();
      });
    });
  ...
  describe('Multiple File Format Support', () => {
    test('handles different resolutions consistently', async () => {
      const files = [TEST_FILES.headMoov480p, TEST_FILES.headMoov720p, TEST_FILES.headMoov1080p];
      ...
      for (const filename of files) {
        ...
        const moovSignature = new TextDecoder().decode(result.moov!.slice(4, 8));
        const ftypSignature = new TextDecoder().decode(result.ftyp!.slice(4, 8));
    ...
    test('handles both head and tail MOOV files', async () => {
      const headFiles = [TEST_FILES.headMoov480p, TEST_FILES.headMoov720p, TEST_FILES.headMoov1080p];
      const tailFiles = [TEST_FILES.tailMoov480p, TEST_FILES.tailMoov720p, TEST_FILES.tailMoov1080p];
  ...
  describe('MP4 Construction Utilities', () => {
    describe('createDummyMdat', () => {
      ...
      test('rejects invalid sizes less than 8 bytes', () => {
        expect(() => createDummyMdat(7)).toThrow('mdat size must be at least 8 bytes');
        expect(() => createDummyMdat(0)).toThrow('mdat size must be at least 8 bytes');
        expect(() => createDummyMdat(-1)).toThrow('mdat size must be at least 8 bytes');
      });
    });
    ...
    describe('buildFakeMp4', () => {
      ...
      test('creates MP4 with correct box order', async () => {
        ...
        expect(fakeMp4.slice(ftyp!.length, ftyp!.length + moov!.length)).toEqual(moov);

        // Verify MDAT comes last and has correct signature
        const mdatStart = ftyp!.length + moov!.length;
        const mdatSignature = new TextDecoder().decode(fakeMp4.slice(mdatStart + 4, mdatStart + 8));
      ...
      test('rejects missing required boxes', async () => {
        ...
        expect(() => buildFakeMp4(null as any, moov!)).toThrow('Both ftyp and moov boxes are required');
        expect(() => buildFakeMp4(ftyp!, null as any)).toThrow('Both ftyp and moov boxes are required');
        expect(() => buildFakeMp4(null as any, null as any)).toThrow('Both ftyp and moov boxes are required');
      });
    });
    ...
    describe('buildMp4FromSegment', () => {
      test('reconstructs valid MP4 from segment data', async () => {
        ...
        expect(mp4.length).toBeGreaterThan(ftyp!.length + moov!.length + segmentData.length);
      ...
      test('creates proper MDAT header for segment data', async () => {
        ...
        const mdatSignature = new TextDecoder().decode(mp4.slice(mdatStart + 4, mdatStart + 8));
        expect(mdatSignature).toBe('mdat');

        // Verify segment data is included
        const actualSegmentData = mp4.slice(mdatStart + 8, mdatStart + 8 + segmentData.length);
      ...
      test('rejects missing required data', async () => {
        ...
        expect(() => buildMp4FromSegment(null as any, moov!, segmentData)).toThrow('Both ftyp and moov boxes are required');
        expect(() => buildMp4FromSegment(ftyp!, null as any, segmentData)).toThrow('Both ftyp and moov boxes are required');
        expect(() => buildMp4FromSegment(ftyp!, moov!, null as any)).toThrow('Segment data is required');
        expect(() => buildMp4FromSegment(ftyp!, moov!, new Uint8Array(0))).toThrow('Segment data is required');
      });
    });
  });
}); 