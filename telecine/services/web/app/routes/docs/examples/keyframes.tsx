...
function KeyframeMarker({
  ...
  const handlePointerDown = (e: React.PointerEvent) => {
    ...
    const timelineElement = (e.target as HTMLElement).closest(
      "[data-timeline-track]",
    );
  ...
  return (
    <div
      className={`absolute w-4 h-4 rounded-full border-2 border-white cursor-pointer transform -translate-x-1/2 -translate-y-1/2 transition-all shadow-sm ${
        isDragging ? "scale-125 shadow-md" : "hover:scale-110"
...
function TimelineTrack({
  ...
  const handleTrackClick = (e: React.MouseEvent) => {
    ...
    const timeMs = Math.max(
      0,
      Math.min((relativeX / rect.width) * totalDuration, totalDuration),
    );
    ...
    <div className="timeline-track-container">
      ...
      <div className="flex items-center gap-4 py-1 relative">
        <div className="w-20 text-sm font-medium text-gray-700 flex-shrink-0">
          {track.property === "offset-distance"
            ? "motion path"
            : track.property}
        </div>
        <div
        ...
        >
          ...
          {track.keyframes.map((keyframe) => (
            <KeyframeMarker
              ...
              onValueChange={(kf, value) =>
                onKeyframeValueChange(track, kf, value)
              }
      ...
      <div className="h-10 ml-24 mr-16 mt-1">
        {(() => {
          ...
          const activeKeyframe = track.keyframes.find(
            (kf) => Math.abs(kf.timeMs - currentTime) < 50,
          );
              ...
              <div className="flex items-center gap-3 h-full">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs font-mono text-blue-600 bg-blue-50 px-2 py-0.5 rounded border">
                    {track.easing === "cubic-bezier(0.68, -0.55, 0.265, 1.55)"
                      ? "back-out"
                      : track.easing ===
...
function generateKeyframesCSS(
  ...
  tracks.forEach((track) => {
    ...
    const keyframeRules = track.keyframes
      .map((kf) => {
  ...
  const pathTrack = tracks.find(
    (track) => track.property === "offset-distance",
  );
      ...
      prevTracks.map((t) =>
        t === track
          ? {
              ...t,
              keyframes: t.keyframes.map((kf) =>
                kf.id === keyframe.id ? { ...kf, value: newValue } : kf,
              ),
            }
          : t,
      ),
      ...
      <div className="grid grid-cols-2 gap-6 p-6 min-h-0 overflow-hidden">
        ...
        <section className="bg-gradient-to-br from-gray-50 to-gray-100 rounded-lg border border-gray-200 flex flex-col">
          <div className="p-4 border-b border-gray-200">
            ...
            <p className="text-sm text-gray-600">
              CSS Motion Path + custom easing for curved motion
            </p>
            <div className="text-xs text-gray-500 mt-1">
              Purple dashed line shows offset-path • Element follows curved path
              naturally
            </div>
          </div>
        ...
        <section className="bg-white rounded-lg border border-gray-200 flex flex-col">
          <div className="p-4 border-b border-gray-200">
            ...
            <p className="text-sm text-gray-600">
              Interactive timeline with playhead synchronization
            </p>
            <div className="text-xs text-gray-500 mt-1 flex justify-between">
              <span>Duration: {duration}ms</span>
              <span className="font-mono">
                Current: {Math.round(currentTime)}ms (
          ...
          <div className="mt-4 p-4 border-t border-gray-200">
            <h4 className="text-sm font-semibold text-gray-800 mb-2">
              Generated CSS (using animation-composition)
            </h4>
            ...
            <p className="text-xs text-gray-500 mt-2">
              Demonstrates curved motion using CSS Motion Path (offset-path) +
              separate transform animations. No manual keyframe generation
              needed!
            </p>
          </div>
          ...
          <div className="p-4 border-t border-gray-200 text-xs text-gray-600">
            <div className="space-y-1">
              <div>
                • <strong>Click</strong> timeline tracks to jump playhead
              </div>
              <div>
                • <strong>Double-click</strong> timeline tracks to add keyframes
              </div>
              <div>
                • <strong>Click</strong> keyframes to jump playhead to that time
              </div>
              <div>
                • <strong>Drag</strong> keyframes to change timing
              </div>
              <div>
                • <strong>Editor appears automatically</strong> when playhead
                matches keyframe time
              </div>
              <div>
                • <strong>CSS Motion Path</strong> creates natural curved motion
                paths
              </div>
              <div>
                • <strong>offset-distance</strong> animates progress along the
                defined path
              </div>
              <div>
                • <strong>Separate transforms</strong> can still be applied
                (rotation, scaling)
              </div>
              <div>
                • <strong>No complex math</strong> - CSS handles all curve
                calculations
              </div>
              <div>
                • Much more intuitive than coordinating X/Y timing functions
              </div>
            </div>
          </div>
        </section>
