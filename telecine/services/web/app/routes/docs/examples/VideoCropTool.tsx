...
function cropToPixelBounds(crop: CropHandle, containerWidth: number, containerHeight: number): BoxBounds {
  return {
...
function pixelBoundsToCrop(bounds: BoxBounds, containerWidth: number, containerHeight: number): CropHandle {
  return {
...
export function VideoCropTool({
  ...
  return (
    <div className={`relative bg-black overflow-hidden border-2 border-gray-300 ${className}`} style={{ width, height }}>
      <Preview className="w-full h-full">
        ...
        >
          ...
          <ResizableBox
            bounds={pixelBounds}
            onBounds-change={(e: CustomEvent<{ bounds: BoxBounds }>) => handleBoundsChange(e.detail.bounds)}
