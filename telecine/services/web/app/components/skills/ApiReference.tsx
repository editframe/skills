...
function AttributeCard({ attr }: { attr: ApiAttribute }) {
  return (
    <div className="border border-black/10 dark:border-white/10 rounded p-3 hover:border-black/20 dark:hover:border-white/20 transition-colors">
      {/* Header */}
      <div className="flex items-start justify-between gap-2 mb-1.5">
        <div className="flex items-baseline gap-2 flex-1 min-w-0">
          <code className="text-sm font-bold text-[var(--ink-black)] dark:text-white">
            {attr.name}
          </code>
          {attr.required && (
        ...
        <code className="text-xs font-mono text-[var(--accent-blue)] dark:text-blue-400 flex-shrink-0">
          {attr.type}
        </code>
      </div>

      {/* Description */}
      <p className="text-sm text-[var(--warm-gray)] dark:text-gray-400 leading-relaxed">
        {attr.description}
      </p>
        ...
        <div className="mt-2 pt-2 border-t border-black/5 dark:border-white/5 flex flex-wrap items-center gap-x-3 gap-y-1.5 text-xs">
          ...
          {attr.default !== undefined && (
            <div className="flex items-center gap-1.5">
              <span className="text-[var(--warm-gray)]/60 dark:text-gray-500 font-medium">
                Default:
              </span>
              <code className="px-1.5 py-0.5 rounded bg-black/5 dark:bg-white/10 text-[var(--ink-black)] dark:text-gray-300 font-mono">
                {String(attr.default)}
              </code>
            </div>
          )}
          ...
          {attr.values && attr.values.length > 0 && (
            <div className="flex items-center gap-1.5 flex-wrap">
              ...
              {attr.values.map((value) => (
                <code
                  key={value}
                  className="px-1.5 py-0.5 rounded bg-black/5 dark:bg-white/10 text-[var(--ink-black)] dark:text-gray-300 font-mono"
                >
                  {value}
                </code>
              ))}
...
export function ApiReference({ api }: { api: ApiMetadata }) {
  ...
  return (
    <div className="not-prose my-8">
      ...
      {hasAttributes && (
        <div className="mb-8">
          <h2 className="text-xl font-bold text-[var(--ink-black)] dark:text-white mb-4 pb-2 border-b border-black/10 dark:border-white/10">
            Attributes
          </h2>
        ...
        <div className="mb-8">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
            {api.methods!.map((method) => (
              ...
              >
                <code className="text-sm font-mono text-[var(--accent-blue)] dark:text-blue-400 block mb-1.5">
                  {method.signature}
                </code>
                <p className="text-sm text-[var(--warm-gray)] dark:text-gray-400 leading-relaxed">
                  {method.description}
                ...
                {method.returns && (
                  <div className="mt-2 pt-2 border-t border-black/5 dark:border-white/5 text-xs text-[var(--warm-gray)]/60 dark:text-gray-500">
