  ...
  describe.runIf(isNativeCanvasApiAvailable())("video export with native rendering", () => {
    test("renderTimegroupToVideo with React TimelineRoot + Configuration (matches design-catalog setup)", async () => {
      ...
      try {
        ...
        root.render(<TimelineRoot id="test-timeline" component={ColorChangingContent} />);

        // Wait for React to render and custom elements to upgrade
        await vi.waitUntil(() => container.querySelector("ef-timegroup") !== null, {
          timeout: 5000,
        });
    ...
    test("captures different frames at different timestamps (design catalog like)", async () => {
      ...
      try {
        ...
        root = createRoot(container);
        root.render(<TimelineRoot id="test-catalog" component={DesignCatalogLikeContent} />);
    ...
    test("clone is created with React content that responds to seek", async () => {
      ...
      try {
        ...
        root = createRoot(container);
        root.render(<TimelineRoot id="test-timeline-2" component={ColorChangingContent} />);
    ...
    test.runIf(isNativeCanvasApiAvailable())(
      ...
      async () => {
        ...
        try {
          // Render the TimelineRoot with our test component
          root = createRoot(container);
          root.render(<TimelineRoot id="test-native" component={ColorChangingContent} />);
    ...
    test("captures different frames using FOREIGNOBJECT path (forced)", async () => {
      ...
      try {
        // Render the TimelineRoot with our test component
        root = createRoot(container);
        root.render(<TimelineRoot id="test-foreignobject" component={ColorChangingContent} />);
