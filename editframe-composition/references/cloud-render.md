...
- **bundleRender(options)** - Build a Vite bundle from a local project directory and return a tar stream ready for upload. Programmatic equivalent of `npx editframe cloud-render`.
  - Returns: Promise<ReadableStream>
- **useRenderData<T>()** - React hook. Returns custom data injected at render time via CLI `--data`, `bundleRender({ renderData })`, or the raw HTML `createRender` payload. Returns undefined when no data is present. Stable reference — reads once on mount.
  - Returns: T | undefined
