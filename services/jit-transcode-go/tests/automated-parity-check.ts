...
async function ffprobe(file: string): Promise<any> {
	try {
		const out = execSync(`ffprobe -v quiet -print_format json -show_format -show_streams "${file}"`, { encoding: "utf8" });
...
async function mp4dump(file: string): Promise<any> {
	try {
		const out = execSync(`mp4dump --format json "${file}" 2>/dev/null`, { encoding: "utf8", maxBuffer: 10 * 1024 * 1024 });
		return JSON.parse(out);
	} catch {
		return null;
	}
...
async function main() {
	...
	if (
		await test("Init segments have correct MSE structure", async () => {
			const ts = await fetch$(`${TS}/api/v1/transcode/high/init.m4s?url=${encodeURIComponent(TEST_VIDEO)}`);
			const go = await fetch$(`${GO}/api/v1/transcode/high/init.m4s?url=${encodeURIComponent(TEST_VIDEO)}`);
			...
			if (tsValid && goValid) {
				return { pass: true, message: `Both have ftyp+moov only (TS:${ts.length}b, Go:${go.length}b)` };
			}
	...
	if (
		await test("Media segments have correct MSE structure", async () => {
			const ts = await fetch$(`${TS}/api/v1/transcode/high/1.m4s?url=${encodeURIComponent(TEST_VIDEO)}`);
			const go = await fetch$(`${GO}/api/v1/transcode/high/1.m4s?url=${encodeURIComponent(TEST_VIDEO)}`);
			...
			if (tsValid && goValid) {
				return { pass: true, message: `Both have moof+mdat only (TS:${(ts.length / 1024 / 1024).toFixed(1)}MB, Go:${(go.length / 1024 / 1024).toFixed(1)}MB)` };
			}
	...
	if (
		await test("Init segments are parseable by ffprobe", async () => {
			...
			if (tsCodec === "h264" && goCodec === "h264") {
				return { pass: true, message: `Both h264 (TS:${tsProbe.streams[0].width}x${tsProbe.streams[0].height}, Go:${goProbe.streams[0].width}x${goProbe.streams[0].height})` };
			}
