...
describe("validateUrlToken", () => {
  ...
  test("rejects mismatched URLs", () => {
    ...
    expect(result.errorDetails!.requestUrl).toBe(
      "https://different.com/video.mp4",
    );
    expect(result.errorDetails!.signedUrl).toBe(
  ...
  test("rejects URLs with different query parameters", () => {
    ...
    const result = validateUrlToken(
      session,
      "https://example.com/video.mp4?token=xyz",
    );
  ...
  test("validates when signed URL is a prefix of request URL", () => {
    ...
    const result = validateUrlToken(
      session,
      "https://example.com/videos/movie1.mp4",
    );
  ...
  test("validates when signed URL is a path prefix", () => {
    ...
    const result = validateUrlToken(
      session,
      "https://example.com/api/v1/transcode/manifest.json",
    );
  ...
  test("rejects when request has query params but signed URL doesn't", () => {
    ...
    const result = validateUrlToken(
      session,
      "https://example.com/videos/movie1.mp4?quality=high",
    );
  ...
  test("validates when both URLs have matching parameters", () => {
    const session = createUrlSession("https://example.com/api/v1/transcode", {
      url: "source.mp4",
    });
    const result = validateUrlToken(
      session,
      "https://example.com/api/v1/transcode/video-1080p/1.mp4?url=source.mp4",
    );
  ...
  test("validates when both URLs have multiple matching parameters", () => {
    ...
    const result = validateUrlToken(
      session,
      "https://example.com/api/transcode?url=source.mp4&quality=hd",
    );
  ...
  test("validates when both URLs have no parameters", () => {
    ...
    const result = validateUrlToken(
      session,
      "https://example.com/api/v1/media/123",
    );
  ...
  test("rejects when request has extra parameters", () => {
    ...
    const result = validateUrlToken(
      session,
      "https://example.com/api/transcode?url=source.mp4&extra=value",
    );
  ...
  test("rejects when request is missing parameters", () => {
    ...
    const result = validateUrlToken(
      session,
      "https://example.com/api/transcode?url=source.mp4",
    );
  ...
  test("rejects when parameter values don't match", () => {
    ...
    const result = validateUrlToken(
      session,
      "https://example.com/api/transcode?url=different.mp4",
    );

    expect(result.isValid).toBe(false);
    expect(result.errorDetails).toBeDefined();
    expect(result.errorDetails!.message).toContain(
      'Parameter value mismatch for "url"',
    );
  });
  ...
  test("handles URL encoding in parameters correctly", () => {
    ...
    const result = validateUrlToken(
      session,
      "https://example.com/api/transcode?url=source%20file.mp4",
    );
  ...
  test("transcode use case - multiple segments share token", () => {
    const session = createUrlSession("https://editframe.com/api/v1/transcode", {
  ...
  test("handles relative URLs by using signed URL's origin", () => {
    ...
    const relativeRequestUrl =
      "/api/v1/transcode/manifest.json?url=http%3A%2F%2Fweb%3A3000%2Fbars-n-tone.mp4";
