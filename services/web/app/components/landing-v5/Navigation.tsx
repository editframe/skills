...
function readLoggedInCookie(): boolean {
  return document.cookie
    .split(";")
    .some((c) => c.trim().startsWith("_logged_in=1"));
}
...
export function Navigation() {
  ...
  return (
    <nav
      ...
      className="fixed top-0 left-0 right-0 z-50 bg-[var(--paper-cream)] border-b-2 border-[var(--ink-black)] dark:border-white"
    >
      <div className="max-w-7xl mx-auto flex items-center justify-between h-16 px-6">
        ...
        <div className="flex items-center gap-3">
          ...
          {isLoggedIn ? (
            ...
            <Link
              to="/auth/login"
              className="max-md:hidden inline-flex px-4 py-2 text-sm font-bold uppercase tracking-wider hover:bg-[var(--poster-gold)] transition-colors"
            >
              Sign in
            </Link>
          )}
          ...
          >
            <div
              className={`w-5 h-0.5 bg-[var(--ink-black)] dark:bg-white transition-transform ${mobileMenuOpen ? "translate-y-[4px] rotate-45" : ""}`}
            />
            <div
              className={`w-5 h-0.5 bg-[var(--ink-black)] dark:bg-white transition-opacity ${mobileMenuOpen ? "opacity-0" : ""}`}
            />
            <div
              className={`w-5 h-0.5 bg-[var(--ink-black)] dark:bg-white transition-transform ${mobileMenuOpen ? "-translate-y-[4px] -rotate-45" : ""}`}
            />
