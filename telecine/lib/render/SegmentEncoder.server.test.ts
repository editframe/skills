...
describe("SegmentEncoder", () => {
  ...
  describe("Per-phase timing and pipelining", () => {
    test("logs framePhaseBreakdown with beginFrame and captureFrame breakdown", async () => {
      const engine = new TestFramegenEngine(480, 270);
      const encoder = createSegmentEncoder({
        engine,
        renderOptions: {
      ...
      expect(mockLogger.info).toHaveBeenCalledWith(
        expect.objectContaining({
          ...
          avgBeginFrameMs: expect.any(Number),
          avgCaptureFrameMs: expect.any(Number),
          avgWriteMs: expect.any(Number),
        }),
        "Frame phase breakdown",
      );
    }, 6000);

    test("beginFrame for next frame overlaps with captureFrame for current frame", async () => {
      const callOrder: string[] = [];
      let captureCallCount = 0;

      const makeFrame = (frameNumber: number) => {
        const frame = Buffer.alloc(480 * (270 + 1) * 4);
        const sampleX = Math.floor(480 / 2);
        const sampleY = 270;
        const pixelOffset = (sampleY * 480 + sampleX) * 4;
        frame[pixelOffset + 2] = (frameNumber >> 16) & 0xff;
        frame[pixelOffset + 1] = (frameNumber >> 8) & 0xff;
        frame[pixelOffset + 0] = frameNumber & 0xff;
        return frame;
      };
      ...
      const engine = {
        ...
        beginFrame: vi.fn().mockImplementation(async (frameNumber: number) => {
          callOrder.push(`beginFrame-${frameNumber}`);
          return Buffer.alloc(0);
        }),
        captureFrame: vi.fn().mockImplementation(async (frameNumber: number) => {
          callOrder.push(`captureFrame-${frameNumber}-start`);
          const idx = captureCallCount++;
          // For frame 0 only: introduce a small async gap so beginFrame(1) can be called
          if (idx === 0) {
            await new Promise<void>((resolve) => {
              // Allow macrotask to run so pipelined beginFrame can be dispatched
              setImmediate(resolve);
            });
          }
          callOrder.push(`captureFrame-${frameNumber}-end`);
          return makeFrame(frameNumber);
        }),
      };

      const encoder = createSegmentEncoder({
        engine: engine as unknown as FramegenEngine,
        renderOptions: {
          durationMs: 200,
          encoderOptions: {
            ...createTestRenderOptions().encoderOptions,
      ...
      // Pipelining: beginFrame(1) should be called before captureFrame(0) ends
      const beginFrame1Index = callOrder.indexOf("beginFrame-1");
      const captureFrame0EndIndex = callOrder.indexOf("captureFrame-0-end");
      expect(beginFrame1Index).toBeGreaterThanOrEqual(0);
      expect(captureFrame0EndIndex).toBeGreaterThanOrEqual(0);
      expect(beginFrame1Index).toBeLessThan(captureFrame0EndIndex);
    }, 6000);
  });
