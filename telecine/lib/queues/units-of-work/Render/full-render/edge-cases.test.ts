...
describe("Render to end of source media", async () => {
  test("renders to end of source media", async ({
    expect,
    barsNTone,
    render,
  }) => {
...
describe("Short Duration Edge Case Regression", () => {
  test("renders very short duration without concat directive errors", async ({
    render,
    expect,
  }) => {
    ...
    await expect(
      render(html, { renderSliceMs: 500 }, "short-duration-11ms"),
    ).resolves.not.toThrow();
  }, 30_000);

  test("renders borderline audio frame duration without errors", async ({
    render,
    expect,
  }) => {
    ...
    await expect(
      render(html, { renderSliceMs: 500 }, "borderline-audio-frame-501ms"),
    ).resolves.not.toThrow();
  });

  test("renders slightly below audio frame duration without errors", async ({
    render,
    expect,
  }) => {
    ...
    await expect(
      render(html, { renderSliceMs: 500 }, "below-audio-frame-20ms"),
    ).resolves.not.toThrow();
  });
});
