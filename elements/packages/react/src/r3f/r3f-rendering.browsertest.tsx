...
describe.runIf(isWebGLAvailable())(
  "Three.js WebGL rendering via addFrameTask",
  () => {
...
describe.runIf(isWebGLAvailable())(
  "OffscreenCanvas + Worker WebGL rendering",
  () => {
  ...
  () => {
    test(
      ...
      async () => {
        ...
        console.log(
          "[Freeze Test] Pause duration:",
          result.pauseDuration,
          "ms",
        );
        console.log(
          "[Freeze Test] Worker frames:",
        ...
        for (let i = 0; i < result.workerResults.length; i++) {
          ...
          assert.exists(
            frame.pixel,
            `Worker frame ${frame.frameId} should have pixel data`,
          );
        ...
        assert.isNotNull(
          result.mainResults,
          "Main thread results should exist",
        );
        assert.isAbove(
