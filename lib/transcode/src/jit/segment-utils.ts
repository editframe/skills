...
import { frameDuration, getClosestAlignedTime, videoFrameDuration, getClosestVideoAlignedTime } from './calculations';
...
export function generateSingleSegmentInfo(
  ...
  const startTimeUs = Math.round(segmentIndex * segmentDuration * TIME_CONSTANTS.MICROSECONDS_PER_MILLISECOND); // Convert ms to us
  ...
  const endTimeUs = Math.min(
    Math.round((segmentIndex + 1) * segmentDuration * TIME_CONSTANTS.MICROSECONDS_PER_MILLISECOND),
    durationUs
  );

  const totalDurationForSegmentCalculation = durationInSeconds ? duration : duration / TIME_CONSTANTS.MILLISECONDS_PER_SECOND;
  const segmentDurationInSeconds = segmentDuration / TIME_CONSTANTS.MILLISECONDS_PER_SECOND;
  const isLast = segmentIndex === Math.ceil(totalDurationForSegmentCalculation / segmentDurationInSeconds) - 1;
  ...
  let startTimeWithPadding = Math.max(alignedStartTime - frameDurationForPadding * framePaddingMultiplier, 0);
  ...
  return {
    index: segmentIndex,
    startTimeUs: (useAlignedTimes || videoFrameRate) ? alignedStartTime : startTimeUs,
    endTimeUs: (useAlignedTimes || videoFrameRate) ? alignedEndTime : endTimeUs,
    isLast,
    actualStartTimeUs: startTimeWithPadding,
    actualDurationUs: (useAlignedTimes || videoFrameRate) ? realDurationUs : (endTimeUs - startTimeUs)
  };
}
...
export async function repackageFragmentedSegment(
  ...
  let baseMediaDecodeTimeMs = segmentInfo.startTimeUs / TIME_CONSTANTS.MICROSECONDS_PER_MILLISECOND; // Convert microseconds to milliseconds
  ...
  const mediaSegmentBytes = repackageMediaSegment(mp4IsoFile, segmentIndex, baseMediaDecodeTimeMs);
