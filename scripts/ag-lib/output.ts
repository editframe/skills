...
export type OutputType = "summary" | "findings" | "plan" | "code_changes" | "analysis" | "decision" | "execution_result";
...
export function readOutputs(planDocumentId?: string, outputType?: OutputType): AgentOutput[] {
  const db = getDb();
  if (planDocumentId && outputType) {
    ...
    return db.prepare("SELECT * FROM agent_outputs ORDER BY created_at ASC").all() as AgentOutput[];
  }
}
