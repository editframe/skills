...
window._ready = true;
    ...
    await page.waitForFunction(
      () => (window as any)._workerDone && (window as any)._mainDone,
      null,
      { timeout: 10000 },
    );

    const activeWorkerResults = await page.evaluate(
      () => (window as any)._workerResults,
    );
    const activeMainResults = await page.evaluate(
    ...
    console.log(
      "  (Page.setWebLifecycleState('frozen') pauses main thread JS)",
    );
    ...
    // Read results
    const frozenWorkerDone = await page.evaluate(
      () => (window as any)._workerDone,
    );
    const frozenWorkerResults = await page.evaluate(
      () => (window as any)._workerResults,
    );
    const frozenMainDone = await page.evaluate(() => (window as any)._mainDone);
    const frozenMainResults = await page.evaluate(
      () => (window as any)._mainResults,
    );
    ...
    if (frozenWorkerResults) {
      for (const r of frozenWorkerResults) {
        console.log(
          `    Worker frame ${r.frameId}: ${JSON.stringify(r.pixel)}`,
        );
      }
    }
    ...
    console.log(
      "  Main thread frames rendered:",
      frozenMainResults?.length ?? 0,
    );
    ...
    if (activeWorkerResults.length === totalFrames) {
      console.log(
        `  PASS: Worker rendered all ${totalFrames} frames when active`,
      );
    } else {
      console.log(
        `  FAIL: Worker only rendered ${activeWorkerResults.length}/${totalFrames} frames when active`,
      );
      allPass = false;
    }
    ...
    if (activeMainResults.length === totalFrames) {
      console.log(
        `  PASS: Main thread rendered all ${totalFrames} frames when active`,
      );
    } else {
      console.log(
        `  FAIL: Main thread only rendered ${activeMainResults.length}/${totalFrames} frames when active`,
      );
      allPass = false;
    }
    ...
    for (const r of activeWorkerResults) {
      ...
      if (expected) {
        const ok =
          r.pixel.r >= expected.r[0] &&
          r.pixel.r <= expected.r[1] &&
          r.pixel.g >= expected.g[0] &&
          r.pixel.g <= expected.g[1] &&
          r.pixel.b >= expected.b[0] &&
          r.pixel.b <= expected.b[1];
        if (ok) {
          console.log(`  PASS: Worker frame ${r.frameId} has correct color`);
        } else {
          console.log(
            `  FAIL: Worker frame ${r.frameId} has wrong color: ${JSON.stringify(r.pixel)}`,
          );
          allPass = false;
        }
      }
    ...
    if (frozenWorkerResults && frozenWorkerResults.length === totalFrames) {
      console.log(
        `  PASS: Worker rendered ALL ${totalFrames} frames during page freeze`,
      );
    } else {
      console.log(
        `  FAIL: Worker only rendered ${frozenWorkerResults?.length ?? 0}/${totalFrames} frames during freeze`,
      );
      allPass = false;
    }
    ...
    if (frozenWorkerResults) {
      for (const r of frozenWorkerResults) {
        ...
        if (expected) {
          ...
          if (ok) {
            console.log(
              `  PASS: Worker frame ${r.frameId} correct during freeze`,
            );
          } else {
            console.log(
              `  FAIL: Worker frame ${r.frameId} wrong during freeze: ${JSON.stringify(r.pixel)}`,
            );
            allPass = false;
          }
        }
    ...
    console.log(
      `  Unfreeze:     T=${unfreezeTime - freezeStartTime}ms (${unfreezeTime})`,
    );

    if (frozenWorkerResults && frozenWorkerResults.length > 0) {
      const workerFirstTs = frozenWorkerResults[0].timestamp;
      const workerLastTs =
        frozenWorkerResults[frozenWorkerResults.length - 1].timestamp;
      const workerSpan = workerLastTs - workerFirstTs;
      console.log(
        `  Worker: first frame at T=${workerFirstTs - freezeStartTime}ms, last at T=${workerLastTs - freezeStartTime}ms (span: ${workerSpan}ms)`,
      );

      // Worker frames should be spread across ~2 seconds (10 * 200ms)
      if (workerSpan > 1000) {
        console.log(
          `  PASS: Worker frames spread across ${workerSpan}ms (rendered DURING freeze)`,
        );
      } else {
    ...
    if (frozenMainResults && frozenMainResults.length > 1) {
      ...
      const mainLastTs =
        frozenMainResults[frozenMainResults.length - 1].timestamp;
      const mainSpan = mainLastTs - mainFirstTs;
      const mainSecondToLast = mainLastTs - mainSecondTs;
      console.log(
        `  Main:   first frame at T=${mainFirstTs - freezeStartTime}ms, last at T=${mainLastTs - freezeStartTime}ms (span: ${mainSpan}ms)`,
      );
      ...
      if (frame2AfterUnfreeze && framesClustered) {
        console.log(
          `  PASS: Main thread frame 2 at T=${mainSecondTs - freezeStartTime}ms (AFTER unfreeze at T=${unfreezeTime - freezeStartTime}ms)`,
        );
        console.log(
          `  PASS: Frames 2-${frozenMainResults.length} clustered in ${mainSecondToLast}ms (catch-up after unfreeze)`,
        );
        console.log(
          "  PROVEN: Main thread timers were FROZEN, then caught up after unfreeze",
        );
      } else if (frame2AfterUnfreeze) {
        console.log(
          `  PASS: Main thread frame 2 after unfreeze (timers were paused)`,
        );
        console.log(`  INFO: Post-unfreeze span: ${mainSecondToLast}ms`);
      } else {
        console.log(
          `  INFO: Main thread frame 2 at T=${mainSecondTs - freezeStartTime}ms (before unfreeze?)`,
        );
    ...
    if (
      frozenWorkerResults &&
      frozenMainResults &&
      frozenWorkerResults.length > 1 &&
      frozenMainResults.length > 1
    ) {
      const workerMedianTs =
        frozenWorkerResults[Math.floor(frozenWorkerResults.length / 2)]
          .timestamp;
      const mainMedianTs =
        frozenMainResults[Math.floor(frozenMainResults.length / 2)].timestamp;
